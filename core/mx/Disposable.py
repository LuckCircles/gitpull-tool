...
from typing import Callable, Deque, Self, Set
...
class Disposable:
    ...
    def __init__(self):
        ...
        if self.dispose.__func__ != Disposable.dispose:
            raise Exception(
                f"You must not to override {self.__class__.__qualname__}.dispose(), use __dispose__ instead."
            )
    ...
    def __str__(self):
        return f"[{self.__class__.__qualname__}]{'[DISPOSED]' if self.__disposed == 2 else '[DISPOSING]' if self.__disposed == 1 else ''}"
