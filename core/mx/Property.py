...
class GetSetProperty(Disposable, IProperty_v[T]):
    ...
    def __init__(
        ...
        if changed_event is not None:
            self._conn = changed_event.listen(
                lambda *_: self._ev.emit(self._getter())
            ).dispose_with(self)
        else:
            self._conn = None
...
class EvaluableProperty(Disposable, IEvaluableProperty_rv[T]):
    ...
    def __init__(self, evaluator: Callable[[], T], filter: Callable[[T], T] = None):
        super().__init__()
