...
class StateChoice(State[T], IStateChoice_v):
    ...
    def __init__(
        self, availuator: Callable[[], Sequence[T]], filter: Callable[[T, T], T] = None
    ):
        """
        ```
        ...
        e_avail = self.__e_avail = EvaluableProperty[Sequence[T]](
            availuator,
            # Filter removes duplicates without sorting
            filter=lambda avail: tuple(dict.fromkeys(avail)),
        ).dispose_with(self)
    ...
    def __filter_func(self, new_value: T, value: T):
        ...
        if new_value is not None and new_value not in avail:
            # Revert old value
            new_value = value
        ...
        if new_value is not None and self.__filter is not None:
            # User filter
