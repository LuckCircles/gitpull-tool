...
from ..lib.math import FAffMat2, FBBox, FLine2f, FVec2, FVec2f
...
def QTransform_from_mat(mat: FAffMat2) -> QTransform:
    mat = mat.as_np()
    return QTransform(
        mat[0, 0], mat[1, 0], 0, mat[0, 1], mat[1, 1], 0, mat[0, 2], mat[1, 2], 1
    )
...
def QPoint_to_np(pt: QPoint) -> np.ndarray:
    return np.int32([pt.x(), pt.y()])


def QPoint_to_FVec2f(pt: QPoint) -> FVec2f:
    return FVec2f(pt.x(), pt.y())


def QPointF_to_FVec2f(pt: QPointF) -> FVec2f:
    return FVec2f(pt.x(), pt.y())


def QPoint_from_np(pt: np.ndarray) -> QPoint:
    return QPoint(*pt.astype(np.int32, copy=False))


def QPoint_from_FVec2(pt: FVec2) -> QPoint:
    return QPoint(int(pt.x), int(pt.y))


def QLine_from_FLine2f(line: FLine2f) -> QLine:
    return QLine(
        int(line.p0.x),
        int(line.p0.y),
        int(line.p1.x),
...
def QPointF_to_np(pt: QPoint) -> np.ndarray:
    return np.float32([pt.x(), pt.y()])


def QPointF_from_np(pt: np.ndarray) -> QPoint:
    return QPointF(*pt.astype(np.float32, copy=False))


def QRect_from_FBBox(r: FBBox) -> QRect:
    return QRect(int(r.x), int(r.y), int(r.width), int(r.height))


def QRectF_from_FBBox(r: FBBox) -> QRectF:
    return QRectF(r.x, r.y, r.width, r.height)


def QRect_to_FBBox(r: QRect) -> FBBox:
    return FBBox(r.x(), r.y(), r.width(), r.height())


def QRectF_to_FBBox(r: QRectF) -> FBBox:
    return FBBox(r.x(), r.y(), r.width(), r.height())
...
def QSize_to_FVec2(size: QSize) -> FVec2f:
    return FVec2f(size.width(), size.height())


def QSize_from_FVec2(pt: FVec2f) -> QSize:
    return QSize(int(pt.x), int(pt.y))


def QSizeF_from_FVec2(pt: FVec2f) -> QSizeF:
