...
class QApplication(QObject):
    ...
    def __init__(self, app_name: str = None, state_path: Path = None):
        ...
        self.__q_clipboard = QClipboard(
            q_clipboard=q_app.clipboard(), wrap_mode=True
        ).dispose_with(self)
        ...
        self.__mx_process_priority = mx.StateChoice[ProcessPriority](
            availuator=lambda: ProcessPriority
        ).dispose_with(self)
        self.__mx_process_priority.listen(
            lambda prio, enter: lib_os.set_process_priority(prio) if enter else ...
        )
        ...
        try:
            state = FDict.from_file(
                self.__state_path,
                Path_func=lambda path: lib_path.abspath(path, self.__rel_path),
            )
        except:
            state = FDict()
        ...
        self._mx_settings.reflect(
            lambda settings, enter, bag=mx.Disposable().dispose_with(
                self
    ...
    def __ref_settings(self, settings: QSettings, enter: bool, bag: mx.Disposable):
        if enter:
            ...
            self.__mx_process_priority.set(
                lib_os.ProcessPriority(
                    settings.state.get(
                        "app_process_priority",
                        lib_os.ProcessPriority.BELOW_NORMAL.value,
                    )
                )
            )
            self.__show_console = settings.state.get(
                "app_show_console", self.__show_console
            )

            self.__objects_settings = objects_settings = settings.state.get(
                "app_objects_settings", FDict()
            ).to_dict()

            for q_obj in self.__objects_settings_subscribers.copy():
                q_obj._mx_settings.set(
                    _QSettingsImpl(FDict(objects_settings.get(q_obj.tree_name, {})))
                )

            settings.ev_update.listen(
                lambda: settings.state.update(
                    {
                        "app_language": self.__mx_language.get(),
                        "app_process_priority": self.__mx_process_priority.get().value,
                        "app_show_console": self.__show_console,
                        "app_objects_settings": FDict(self.__objects_settings)
                        | {
                            q_obj.tree_name: (
                                q_obj._mx_settings.get().ev_update.emit(),
                                q_obj._mx_settings.get().state,
                            )[-1]
    ...
    def _obj_settings_subscribe(self, q_obj: QObject):
        ...
        q_obj._mx_settings.set(
            _QSettingsImpl(FDict(self.__objects_settings.get(q_obj.tree_name, {})))
        )
    ...
    @property
    def q_clipboard(self) -> QClipboard:
        return self.__q_clipboard
    ...
    def save_state(self):
        ...
        if self.__state_path is not None:
            ...
            FDict(q_settings.state).dump_to_file(
                self.__state_path,
