...
class QFileDialog(QObject):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            q_object=q_init("q_file_dialog", qt.QFileDialog, **kwargs), **kwargs
        )
        ...
        self._mx_settings.reflect(
            lambda setting, enter, bag=mx.Disposable().dispose_with(
                self
            ): self.__ref_settings(setting, enter, bag)
        )

    @property
    def q_file_dialog(self) -> qt.QFileDialog:
        return self.q_object

    @property
    def mx_accepted(self) -> mx.IEvent0_rv:
        return self.__mx_accepted

    @property
    def selected_files(self) -> Sequence[Path]:
        return tuple(Path(x) for x in self.q_file_dialog.selectedFiles())

    @property
    def directory(self) -> Path:
        return Path(self.q_file_dialog.directory().path())
    ...
    def __ref_settings(self, settings: QSettings, enter: bool, bag: mx.Disposable):
        if enter:
            ...
            self.__mx_accepted.listen(
                lambda: settings.state.set("dir", self.directory)
            ).dispose_with(bag)
        else:
            bag.dispose_items()
    ...
    def set_filter(self, filter: str):
        if (disp := getattr(self, "_QLineEdit_text_disp", None)) is not None:
            disp.dispose()
        self._QLineEdit_text_disp = (
            QApplication.instance()
            .mx_language.reflect(
                lambda lang: self.q_file_dialog.setNameFilter(lx.L(filter, lang))
            )
            .dispose_with(self)
        )
        return self
