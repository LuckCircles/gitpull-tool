...
class QTapeItemView(QVBox):
    ...
    def __init__(self):
        ...
        self._mx_current_item_id = mx.Property[int](
            -1,
            defer=lambda new_value, value, prop: self.apply_model(
                self._m.set_current_item_id(new_value)
            ),
        ).dispose_with(self)
        ...
        self._q_scrollbar_conn = self._q_scrollbar.mx_value.listen(
            lambda value: self.apply_model(self._m.scroll_to(value))
        )

        (self.add(self._q_canvas).add(self._q_scrollbar.v_compact()))
...
class _Canvas(QWidget):
    ...
    def _minimum_size_hint(self) -> QSize:
        ...
        return qt.QSize_from_FVec2(
            m.item_size + FVec2f(m.item_spacing * 2, m.item_spacing * 2)
        )
        ...
        if m.item_count != 0:
            for item_id in range(m.v_item_start, m.v_item_start + m.v_item_count):
                ...
                if current_item_id == item_id:
                    select_rect = item_rect.adjusted(
                        -item_spacing, -item_spacing, item_spacing, item_spacing
                    )
