...
class _QIconWidget(qt.QWidget):
    ...
    def _update(self):
        ...
        if (icon := self._icon) is not None:
            ...
            self._pixmap_rect = qt.QRect_center_in(
                qt.QRect(0, 0, size.width(), size.height()), rect
            )
        else:
...
class QIconWidget(QWidget):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            q_widget=q_init("q_icon_widget", _QIconWidget, **kwargs), **kwargs
        )

    @property
    def q_icon_widget(self) -> _QIconWidget:
        return self.q_widget

    @property
    def icon(self) -> qt.QIcon:
        return self.q_icon_widget.get_icon()
