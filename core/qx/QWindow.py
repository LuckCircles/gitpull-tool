...
class QWindow(QVBox):
    ...
    def __init__(self, **kwargs):
        ...
        self._mx_settings.reflect(
            lambda settings, enter: self._apply_state(settings.state) if enter else ...
        )

        # self.q_widget.setBaseSize(16,16)

    @property
    def mx_window_key_press(self) -> mx.IEvent1_rv[qt.QKeyEvent]:
        return self.__mx_window_key_press

    @property
    def mx_window_key_release(self) -> mx.IEvent1_rv[qt.QKeyEvent]:
        return self.__mx_window_key_release

    @property
    def mx_window_leave(self) -> mx.IEvent0_rv:
        return self.__mx_window_leave

    @property
    def q_window_handle(self) -> qt.QWindow | None:
        return self.__q_window

    @property
    def q_window_icon(self) -> qt.QIcon:
        return self.q_widget.windowIcon()

    @property
    def window_flags(self) -> qt.Qt.WindowType:
        return self.q_widget.windowFlags()
    ...
    def set_title(self, title: str | None = None):
        if (disp := getattr(self, "_QWindow_title_disp", None)) is not None:
            disp.dispose()
        self._QWindow_title_disp = (
            QApplication.instance()
            .mx_language.reflect(
                lambda lang: self.q_widget.setWindowTitle(lx.L(title, lang))
            )
            .dispose_with(self)
        )
        return self
    ...
    def _apply_state(self, state: HFDict = None):
        ...
        if self.visible:
            ...
            if reload or state_first_time:
                ...
                window_state = self.__state.get(
                    "windowState", qt.Qt.WindowState.WindowNoState.value
                )
                ...
                window_state_m = (
                    (window_state & qt.Qt.WindowState.WindowMaximized)
                    == qt.Qt.WindowState.WindowMaximized
                    or (window_state & qt.Qt.WindowState.WindowMinimized)
                    == qt.Qt.WindowState.WindowMinimized
                    or (window_state & qt.Qt.WindowState.WindowFullScreen)
                    == qt.Qt.WindowState.WindowFullScreen
                )
                ...
                if (
                    self.window_flags & qt.Qt.WindowType.MSWindowsFixedSizeDialogHint
                ) == 0:
                ...
                if not window_state_m and pos is not None:
                    ...
                    widget_width, widget_height = (
                        self.q_widget.size().width(),
                        self.q_widget.size().height(),
                    )
                    self.q_widget.move(
    ...
    def _move_event(self, ev: qt.QMoveEvent):
        ...
        if self.visible:
            ...
            if (
                (v & qt.Qt.WindowState.WindowMaximized)
                == qt.Qt.WindowState.WindowNoState
                and (v & qt.Qt.WindowState.WindowMinimized)
                == qt.Qt.WindowState.WindowNoState
                and (v & qt.Qt.WindowState.WindowFullScreen)
                == qt.Qt.WindowState.WindowNoState
            ):
