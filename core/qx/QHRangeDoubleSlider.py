...
class QHRangeDoubleSlider(QHRangeSlider):
    def __init__(self):
        ...
        self.__mx_values = mx.Property[Tuple[float, float]](
            (0, 1.0),
            defer=lambda n, o, prop, super=super(): super.mx_values.set(
                (int(n[0] * self.__decimals_pow), int(n[1] * self.__decimals_pow))
            ),
        ).dispose_with(self)

        super().mx_values.listen(
            lambda v: self.__mx_values._set(
                (v[0] / self.__decimals_pow, v[1] / self.__decimals_pow)
            )
        )
    ...
    def set_range(self, minimum: float, maximum: float):
        super().set_range(
            int(minimum * self.__decimals_pow), int(maximum * self.__decimals_pow)
        )
        return self
