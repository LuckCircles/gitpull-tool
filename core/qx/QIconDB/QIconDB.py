...
Size_to_icon_size = {Size.XXL: 64, Size.XL: 48, Size.L: 32, Size.M: 24, Size.S: 16}
...
class QIconDB(mx.Disposable):
    ...
    def _get_image(self, icon: IconDB) -> FImage:
        ...
        if result is None:
            result = self._cached_np_image[key] = FImage.from_file(
                Path(__file__).parent / "assets" / (icon.name + ".png")
            ).f32()
        return result
    ...
    def cursor(
        ...
        if result is None:
            ...
            base_mask = (
                img.ch1_from_a()
                .resize(cursor_size, cursor_size, smooth=True)
                .pad(stroke_width)
            )
            base_mask_wider = base_mask.dilate(stroke_width)

            stroke_mask = base_mask_wider - base_mask
            colored_stroke = stroke_mask.ch(3) * FImage.full_f32_like(
                stroke_mask, stroke_color.getRgbF()[:3]
            )

            colored_base = base_mask.ch(3) * FImage.full_f32_like(
                base_mask, color.getRgbF()[:3]
            )

            np_cursor = FImage.from_bgr_a(
                colored_base + colored_stroke, base_mask_wider
            )
