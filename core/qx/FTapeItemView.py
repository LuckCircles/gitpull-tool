...
class FTapeItemView(FBaseItemView):
    ...
    @cached_property
    def v_grid_item_start(self) -> int:
        return self._current_item_id - (self.v_row_count * self.v_col_count) // 2

    @cached_property
    def v_grid_item_end(self) -> int:
        return self._current_item_id + (self.v_row_count * self.v_col_count) // 2

    @cached_property
    def v_item_start(self) -> int:
        return max(0, min(self.v_grid_item_start, self.item_count - 1))

    @cached_property
    def v_item_end(self) -> int:
        return max(0, min(self.v_grid_item_end, self.item_count - 1))

    @cached_property
    def v_item_count(self) -> int:
        return self.v_item_end - self.v_item_start + 1

    @cached_property
    def v_grid_start_x(self) -> float:
        return self.cli_size.x / 2 - (self.v_col_count // 2 + 0.5) * self.col_width

    @cached_property
    def v_grid_start_y(self) -> float:
        return self.cli_size.y / 2 - (self.v_row_count // 2 + 0.5) * self.row_height
    ...
    def get_item_cli_box(self, item_id: int) -> FBBoxf:
        ...
        x = (
            self.v_grid_start_x
            + (diff_idx % self.v_col_count) * self.col_width
            + self.item_spacing // 2
        )
        y = (
            self.v_grid_start_y
            + (diff_idx // self.v_col_count) * self.row_height
            + self.item_spacing // 2
        )
