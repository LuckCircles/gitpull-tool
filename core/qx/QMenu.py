...
class QMenu(QWidget):
    ...
    @property
    def q_menu(self) -> qt.QMenu:
        return self.q_widget

    @property
    def mx_about_to_show(self) -> mx.IEvent0_rv:
        return self._mx_about_to_show
    ...
    def set_title(self, text: str | None):
        if (disp := getattr(self, "_QAction_title_disp", None)) is not None:
            disp.dispose()
        self._QAction_title_disp = (
            QApplication.instance()
            .mx_language.reflect(lambda lang: self.q_menu.setTitle(lx.L(text, lang)))
            .dispose_with(self)
        )
        return self
