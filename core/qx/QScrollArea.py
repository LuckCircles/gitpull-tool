...
class QVScrollArea(QWidget):
    def __init__(self, min_width_from_widget=True, **kwargs):
        super().__init__(
            q_widget=q_init("q_scroll_area", qt.QScrollArea, **kwargs), **kwargs
        )
        ...
        q_scroll_area.setVerticalScrollBarPolicy(
    ...
    @property
    def q_scroll_area(self) -> qt.QScrollArea:
    ...
    def _minimum_size_hint(self) -> qt.QSize:
        ...
        if self._min_width_from_widget:
            if (widget := self.q_scroll_area.widget()) is not None:
                ...
                min_size.setWidth(
                    widget_min_size.width()
                    + self.q_style.pixelMetric(qt.QStyle.PixelMetric.PM_ScrollBarExtent)
                    + 2
                )
