...
class QCheckBoxMxMultiChoice(QVBox):
    def __init__(
        self, mc: mx.IMultiChoice_v, stringifier: Callable[[Any], str] = None, **kwargs
    ):
        ...
        mc.mx_avail.reflect(lambda new_avail: self._ref_avail(new_avail)).dispose_with(
            self
        )
    ...
    def update_items(self):
        ...
        for v in avail:
            q_checkbox = (
                QCheckBox().set_text(self._stringifier(v)).set_checked(v in values)
            )
            q_checkbox.mx_toggled.listen(
                lambda checked, v=v: (
                    self._mc.update_added(v) if checked else self._mc.update_removed(v)
                )
            )
