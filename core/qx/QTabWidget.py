...
class QTab(QVBox):
    ...
    def set_title(self, title: str):
        if (disp := getattr(self, "_QTab_title_disp", None)) is not None:
            disp.dispose()
        self._QTab_title_disp = (
            QApplication.instance()
            .mx_language.reflect(
                lambda lang: (q_tab_widget := self._owner.q_tab_widget).setTabText(
                    q_tab_widget.indexOf(self.q_widget), lx.L(title, lang)
                )
            )
            .dispose_with(self)
        )
...
class QTabWidget(QWidget):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            q_widget=q_init("q_tab_widget", qt.QTabWidget, **kwargs), **kwargs
        )

        q_tab_widget = self.q_tab_widget

        self.__mx_current_changed = QEvent1[int](
            q_tab_widget.currentChanged
        ).dispose_with(self)

        self._settings_bag = mx.Disposable()
        self._mx_settings.reflect(
            lambda settings, enter, bag=self._settings_bag: self.__ref_settings(
                settings, enter, bag
            )
        )
    ...
    @property
    def q_tab_widget(self) -> qt.QTabWidget:
        return self.q_widget

    @property
    def mx_current_changed(self) -> QEvent1[int]:
        return self.__mx_current_changed

    @property
    def current_index(self) -> int:
        return self.q_tab_widget.currentIndex()
    ...
    def __ref_settings(self, settings: QSettings, enter: bool, bag: mx.Disposable):
        if enter:
            ...
            self.__mx_current_changed.listen(
                lambda idx: settings.state.set("current_index", idx)
            ).dispose_with(bag)
        else:
            bag.dispose_items()
