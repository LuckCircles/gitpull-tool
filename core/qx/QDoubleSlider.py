...
class QDoubleSlider(QSlider):
    def __init__(self, **kwargs):
        ...
        self.__mx_value = mx.GetSetProperty[float](
            lambda: QDoubleSlider.get_value(self),
            lambda val: QDoubleSlider.set_value(self, val),
            super().mx_value.event,
        ).dispose_with(self)
    ...
    def get_value(self) -> float:
        return (super().get_value() * self._step) + self._minimum

    def set_value(self, value: float):
        return super().set_value(int((value - self._minimum) / self._step))

    def set_decimals(self, decimals: int):
        self._set_config(decimals=decimals)

    def set_step(self, step: float):
        self._set_config(step=step)

    def set_minimum(self, minimum: float):
        self._set_config(minimum=minimum)

    def set_maximum(self, maximum: float):
        self._set_config(maximum=maximum)

    def _set_config(self, minimum=None, maximum=None, step=None, decimals=None):
