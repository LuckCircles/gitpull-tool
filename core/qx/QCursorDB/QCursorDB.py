    ...
    def _get(self, cursor: CursorDB, size: Size, out_cls):
        ...
        if result is None:
            if issubclass(out_cls, qt.QPixmap):
                ...
                pixmap = qt.QPixmap(
                    str(Path(__file__).parent / "assets" / (cursor.name + ".png"))
                )
                pixmap = pixmap.scaled(
                    icon_size,
                    icon_size,
                    mode=qt.Qt.TransformationMode.SmoothTransformation,
                )
                result = self._cached[key] = pixmap

            elif issubclass(out_cls, qt.QCursor):
                result = self._cached[key] = qt.QCursor(
                    self._get(cursor, size, qt.QPixmap)
                )
            else:
