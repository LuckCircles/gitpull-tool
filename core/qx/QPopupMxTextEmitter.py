...
class QPopupMxTextEmitter(QObject):
    def __init__(
        self,
        text_emitter: mx.ITextEmitter_v | Sequence[mx.ITextEmitter_v],
        title: str = None,
    ):
        """ """
        super().__init__()
    ...
    def show_popup(self):
        if self._wnd is None:
            self._text_edit = (
                QTextEdit().set_font(FontDB.FixedWidth).set_read_only(True)
            )
            self._wnd = (
                QWindow()
                .set_parent(self)
                ...
                .add(
                    ...
                    .inline(
                        lambda btn: btn.mx_clicked.listen(lambda: self._wnd.dispose())
                    )
                )
                .show()
            )
            ...
            mx.CallOnDispose(
                lambda: (setattr(self, "_wnd", None), setattr(self, "_text_edit", None))
            ).dispose_with(self._wnd)
