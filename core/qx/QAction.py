...
class QAction(QObject):
    ...
    @property
    def mx_triggered(self) -> mx.IEvent0_rv:
        return self._mx_triggered

    @property
    def q_action(self) -> qt.QAction:
    ...
    def set_text(self, text: str | None):
        if (disp := getattr(self, "_QAction_text_disp", None)) is not None:
            disp.dispose()
        self._QAction_text_disp = (
            QApplication.instance()
            .mx_language.reflect(lambda lang: self.q_action.setText(lx.L(text, lang)))
            .dispose_with(self)
        )
        return self
