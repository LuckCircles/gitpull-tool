...
class QCachedTapeItemView(QTapeItemView):
    def __init__(
        self,
        fut_get_item_pixmap: Callable[[int, qt.QSize], ax.Future[qt.QPixmap]] = None,
    ):
        super().__init__()
    ...
    def apply_model(self, new_m: FTapeItemView):
        ...
        if new_m.item_size != m.item_size or new_m.item_count != m.item_count:
            # Recache all items
            self.update_items()
    ...
    def _bg_cacher(self):
        ...
        while True:
            ...
            for cached_item_id in cached:
                if (
                    cached_item_id < item_scope_start
                    or cached_item_id >= item_scope_end
                ):
    ...
    def _caching_task(self, item_id: int):
        ...
        yield ax.wait(
            fut := self._fut_get_item_pixmap(
                item_id, qt.QSize_from_FVec2(self._m.item_size)
            )
        )
