    ...
    def __ref_settings(self, settings, enter, bag: mx.Disposable):
        if enter:
            ...
            self.add(
                QVBox()
                .dispose_with(bag)
                .add(QVBox().add(self._q_caption.hide(), align=Align.BottomE))
                .add(self._q_progress_bar.set_minimum(0))
                .add(QVBox())
            )

            self._progress.mx_active.reflect(
                lambda active, enter, bag=mx.Disposable().dispose_with(
                    bag
                ): self._ref_active(active, enter, bag)
            ).dispose_with(bag)
        else:
            bag.dispose_items()
    ...
    def _ref_model(self, model: mx.Progress.FModel, old_model: mx.Progress.FModel):
        ...
        if changed_progress:
            if model.is_infinity:
                ...
                if self._show_it_s:
                    self._q_progress_bar.set_format(
                        f"%v / %m ({model.it_s:.1f} @(it_s))"
                    )
                else:
                    self._q_progress_bar.set_format(f"%v / %m")

                self._q_progress_bar.set_maximum(model.progress_max).set_value(
                    model.progress
                )
