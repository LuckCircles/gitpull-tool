...
class QLineEdit(QWidget):
    def __init__(self, **kwargs):
        super().__init__(
            q_widget=q_init("q_line_edit", qt.QLineEdit, **kwargs), **kwargs
        )

        q_line_edit = self.q_line_edit

        mx_editing_finished = self._mx_editing_finished = QEvent0(
            q_line_edit.editingFinished
        ).dispose_with(self)
        mx_editing_finished.listen(lambda: self.clear_focus())

        self._mx_text = mx.GetSetProperty[str](
            self.get_text,
            self.set_text,
            QEvent1[str](q_line_edit.textChanged).dispose_with(self),
        ).dispose_with(self)

        self.set_placeholder_text(None)

    @property
    def mx_text(self) -> mx.IProperty_v[str]:
        return self._mx_text

    @property
    def mx_editing_finished(self) -> mx.IEvent0_rv:
        return self._mx_editing_finished

    @property
    def q_line_edit(self) -> qt.QLineEdit:
    ...
    def set_placeholder_text(self, text: str | None):
        ...
        self._QLineEdit_text_disp = (
            ...
            .mx_language.reflect(
                lambda lang: self.q_line_edit.setPlaceholderText(
