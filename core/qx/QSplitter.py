...
class QSplitter(QWidget):
    def __init__(self, **kwargs):
        super().__init__(
            q_widget=q_init("q_splitter", qt.QSplitter, **kwargs), **kwargs
        )

        self.__default_sizes = []

        q_splitter = self.q_splitter
        self.__mx_splitter_moved = QEvent2[int, int](
            q_splitter.splitterMoved
        ).dispose_with(self)
    ...
    @property
    def q_splitter(self) -> qt.QSplitter:
        return self.q_widget

    @property
    def mx_splitter_moved(self) -> mx.IEvent2_rv[int, int]:
        return self.__mx_splitter_moved

    @property
    def sizes(self) -> List[int]:
        return self.q_splitter.sizes()
    ...
    def __ref_settings(self, settings: QSettings, enter: bool, bag: mx.Disposable):
        if enter:
            ...
            self.__mx_splitter_moved.listen(
                lambda *_: (
                    settings.state.set("sizes", self.sizes) if self.visible else ...
                )
            ).dispose_with(bag)
        else:
            bag.dispose_items()
