...
class QProgressBar(QWidget):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            q_widget=q_init("q_progress_bar", qt.QProgressBar, **kwargs), **kwargs
        )

        q_progress_bar = self.q_progress_bar

        self._mx_value = mx.GetSetProperty[int](
            self.get_value,
            self.set_value,
            QEvent1[int](q_progress_bar.valueChanged).dispose_with(self),
        ).dispose_with(self)

        self.set_orientation(Orientation.Horizontal)

    @property
    def q_progress_bar(self) -> qt.QProgressBar:
    ...
    def get_value(self) -> int:
        return self.q_progress_bar.value()
    ...
    def set_format(self, format: str):
        ...
        if (disp := getattr(self, "_QProgressBar_text_disp", None)) is not None:
            disp.dispose()
        self._QProgressBar_text_disp = (
            QApplication.instance()
            .mx_language.reflect(
                lambda lang: self.q_progress_bar.setFormat(lx.L(format, lang))
            )
            .dispose_with(self)
        )
