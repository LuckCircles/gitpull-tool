...
class QCollapsibleBarButton(QAbstractButton):
    ...
    def _paint_event(self, ev: qt.QPaintEvent):
        ...
        style.drawPrimitive(
            (
                qt.QStyle.PrimitiveElement.PE_IndicatorArrowDown
                if self.is_checked()
                else qt.QStyle.PrimitiveElement.PE_IndicatorArrowRight
            ),
            opt,
            qp,
        )
...
class QCollapsibleVBox(QVBox):
    ...
    def is_opened(self) -> bool:
        return self.__bar_btn.is_checked()
    ...
    def open(self):
        return self.set_opened(True)

    def close(self):
        return self.set_opened(False)
    ...
    def __ref_settings(self, settings: QSettings, enter: bool, bag: mx.Disposable):
        if enter:
            ...
            self.__bar_btn.mx_toggled.listen(
                lambda checked: settings.state.set("opened", checked)
            ).dispose_with(bag)
        else:
            bag.dispose_items()
