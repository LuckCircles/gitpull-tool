...
class FBaseWorldViewWidget(FBaseWidget):
    ...
    @property
    def cli2w_mat(self) -> FAffMat2:
        return self.w2cli_mat.inverted
    ...
    def mouse_lbtn_down(
        ...
        if not (new_self is self):
            ...
            new_self._mouse_l_down_world_pt = self.cli2w_mat.map(
                new_self.mouse_l_down_cli_pt
            )
        return new_self
    ...
    def mouse_lbtn_up(
        ...
        new_self = super().mouse_lbtn_up(
            cli_pt, ctrl_pressed=ctrl_pressed, shift_pressed=shift_pressed
        )
        if not (new_self is self):
            new_self = new_self.clone()
    ...
    def mouse_mbtn_down(
        ...
        if not (new_self is self):
            new_self = new_self.clone()
            new_self._mouse_m_down_world_pt = self.cli2w_mat.map(
                new_self.mouse_m_down_cli_pt
            )
        return new_self
    ...
    def mouse_rbtn_down(
        ...
        if not (new_self is self):
            new_self = new_self.clone()
            new_self._mouse_r_down_world_pt = self.w2cli_mat.inverted.map(
                new_self.mouse_r_down_cli_pt
            )
        return new_self
