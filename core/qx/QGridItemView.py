...
class QGridItemView(QHBox):
    ...
    def __init__(self):
        ...
        q_scrollbar = self._q_scrollbar = (
            QScrollBar().set_orientation(Orientation.Vertical).h_compact()
        )
        self._q_scrollbar_conn = q_scrollbar.mx_value.listen(
            lambda value: self.apply_model(self._m.scroll_to_value(value))
        )

        (self.add(self._q_canvas).add(self._q_scrollbar))

        self._m_update_task()

        QApplication.instance().mx_language.reflect(
            lambda lang: setattr(self, "_lang", lang)
        ).dispose_with(self)

    @property
    def model(self) -> FGridItemView:
        return self._m

    @property
    def mx_model(self) -> mx.IProperty_rv[FGridItemView]:
        return self._mx_model

    @property
    def mx_selected_items(self) -> mx.IProperty_rv[FIndices]:
        return self._mx_selected_items

    @property
    def mx_marked_items(self) -> mx.IProperty_rv[FIndices]:
        return self._mx_marked_items
    ...
    def apply_model(self, new_m: FGridItemView):
        ...
        upd = (
            new_m.cli_size != m.cli_size
            or new_m.item_count != m.item_count
            or changed_scroll_value
            or new_m.selecting_mode != m.selecting_mode
            or (new_m.selecting_mode != 0 and new_m.mouse_cli_pt != m.mouse_cli_pt)
            or changed_selected_idxs
            or changed_marked_idxs
        )

        upd_geo = new_m.item_size != m.item_size or new_m.item_spacing != m.item_spacing

        if new_m.scroll_value_max != m.scroll_value_max:
            if new_m.scroll_value_max != 0:
                with self._q_scrollbar_conn.disabled_scope():
                    self._q_scrollbar.set_minimum(0).set_maximum(
                        new_m.scroll_value_max
                    ).show()
            else:
...
class _Canvas(QWidget):
    ...
    def _resize_event(self, ev: qt.QResizeEvent):
        ...
        self._host.apply_model(
            self._host._m.set_cli_size(ev.size().width(), ev.size().height())
        )
    ...
    def _mouse_press_event(self, ev: qt.QMouseEvent):
        ...
        if ev.button() == qt.Qt.MouseButton.LeftButton:
            self._host.apply_model(
                self._host._m.mouse_lbtn_down(
                    qt.QPoint_to_FVec2f(ev.pos()),
                    ctrl_pressed=qt.QApplication.keyboardModifiers()
                    == qt.Qt.KeyboardModifier.ControlModifier,
                    shift_pressed=qt.QApplication.keyboardModifiers()
    ...
    def _mouse_release_event(self, ev: qt.QMouseEvent):
        ...
        if ev.button() == qt.Qt.MouseButton.LeftButton:
            self._host.apply_model(
                self._host._m.mouse_lbtn_up(qt.QPoint_to_FVec2f(ev.pos()))
            )

    def _wheel_event(self, ev: qt.QWheelEvent):
        super()._wheel_event(ev)
        self._host.apply_model(
            self._host._m.mouse_wheel(
                qt.QPointF_to_FVec2f(ev.position()),
                ev.angleDelta().y(),
                ctrl_pressed=qt.QApplication.keyboardModifiers()
                == qt.Qt.KeyboardModifier.ControlModifier,
                shift_pressed=qt.QApplication.keyboardModifiers()
    ...
    def _paint_event(self, ev: qt.QPaintEvent):
        ...
        if m.item_count == 0:
            ...
            qp.drawText(
                ...
                lx.L("- @(no_items) -", self._host._lang),
            )
        else:
            ...
            if m.v_item_count != 0:
                ...
                for item_id in range(m.v_item_start, m.v_item_start + m.v_item_count):
                    ...
                    selected = (
                        (m._selecting_mode == 0 and selected)
                        or (
                            m._selecting_mode == 1
                            and (
                                (not selected and idx_in_aabb)
                                or (selected and not idx_in_aabb)
                            )
                        )
                    ...
                    if selected:
                        ...
                        item_outter_rect = item_rect.marginsAdded(
                            qt.QMargins(margin, margin, margin, margin)
                        )
                ...
                if m._selecting_mode != 0:
                    qp.fillRect(
                        qt.QRect_from_FBBox(m.selection_cli_box),
                        qt.QColor(255, 255, 255, 127),
                    )
