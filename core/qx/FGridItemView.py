...
class FGridItemView(FBaseItemView):
    ...
    def mouse_lbtn_down(
        self, cli_pt: FVec2f, ctrl_pressed=False, shift_pressed=False
    ) -> Self:
            ...
            if (mouse_grid_item_id := self.mouse_grid_item_id) is not None:
                ...
                if (selection_anchor := self._selection_anchor) is not None:
                    ...
                    if mouse_grid_item_id <= a_min:
                        self = self.select(
                            FIndices.from_range(mouse_grid_item_id, a_min + 1),
                            update_selection_anchor=False,
                        )
                    elif mouse_grid_item_id >= a_max:
                        self = self.select(
                            FIndices.from_range(a_max, mouse_grid_item_id + 1),
                            update_selection_anchor=False,
                        )
        else:
            self._selecting_mode = 2
            ...
            if (aabb := self.selection_aabb) is not None:
                ...
                for row in range(max(0, row_aa), min(row_bb + 1, self.row_count)):
                    for col in range(max(0, col_aa), min(col_bb + 1, v_col_count)):
                        ...
                        if (
                            self._selecting_mode == 1
                            and item_id in self._selected_items
                        ):
                ...
                self = self.select(
                    (self.selected_items | selected).difference(unselected)
                )
    ...
    def mouse_wheel(
        self, cli_pt: FVec2f, delta: float, ctrl_pressed=False, shift_pressed=False
    ) -> Self:
    ...
    def update(self, time_delta: float) -> Self:
        ...
        if self.mouse_l_down_cli_pt is not None:
            ...
            if view_row_diff != 0.0:
                ...
                self = self.scroll_to_row(
                    self.scroll_value
                    + sign * (6 + abs(view_row_diff) ** 4) * time_delta
                )
    ...
    def set_item_count(self, item_count: int) -> Self:
        ...
        if not (new_self is self):
            ...
            new_self._selected_items = new_self._selected_items.discard_ge(
                new_self.item_count
            )
            new_self._marked_items = new_self._marked_items.discard_ge(
                new_self.item_count
            )
        return new_self
    ...
    def col_width(self) -> int:
        return self.item_size.x + self.item_spacing

    @cached_property
    def row_height(self) -> int:
        return self.item_size.y + self.item_spacing

    @cached_property
    def row_count(self) -> int:
        return math.ceil(self.item_count / self.v_col_count)

    @cached_property
    def scroll_value(self) -> float:
        return max(0, min(self._scroll_value, self.scroll_value_max))

    @cached_property
    def scroll_value_max(self) -> float:
        return max(0, self.row_count - math.floor(self.v_row_count))

    @cached_property
    def v_col_count(self) -> int:
        return max(1, int(self.cli_size.x // self.col_width))

    @cached_property
    def v_row_start(self) -> float:
        return self.scroll_value

    @cached_property
    def v_row_count(self) -> float:
        return self.cli_size.y / self.row_height

    @cached_property
    def v_row_end(self) -> float:
        return self.v_row_start + self.v_row_count

    @cached_property
    def v_item_start(self) -> int:
        return int(
            max(
                0,
                min(
                    math.floor(self.scroll_value) * self.v_col_count,
                    self.item_count - 1,
                ),
            )
        )
    ...
    @cached_property
    def v_item_count(self) -> int:
        return int(
            max(
                0,
                min(math.ceil(self.v_row_end) * self.v_col_count, self.item_count)
                - self.v_item_start,
            )
        )

    @cached_property
    def v_items(self) -> FIndices:
        return FIndices.from_range(
            self.v_item_start, self.v_item_start + self.v_item_count
        )
    ...
    def selection_aabb(self) -> Tuple[int, int, int, int] | None:
        if (
            (self._selecting_mode != 0)
            and (mouse_l_down_world_pt := self.mouse_l_down_world_pt) is not None
            and (mouse_world_pt := self.mouse_world_pt) is not None
        ):
    ...
    def selection_cli_box(self) -> FBBoxf:
        """if selecting_mode != 0"""
        return FBBoxf(
            [self.w2cli_mat.map([self.mouse_l_down_world_pt])[0], self.mouse_cli_pt]
        )

    @property
    def selected_items(self) -> FIndices:
    ...
    @property
    def marked_items(self) -> FIndices:
        return self._marked_items

    def is_selected(self, item_id: int) -> bool:
        return item_id in self._selected_items

    def is_marked(self, item_id: int) -> bool:
        return item_id in self._marked_items

    @cached_method
    def is_selected_all(self) -> bool:
        return self._selected_items.count == self.item_count

    @cached_method
    def is_marked_all(self) -> bool:
        return self._marked_items.count == self.item_count
    ...
    def select(
        self, indices: FIndices | Iterable | int | None, update_selection_anchor=True
    ) -> Self:
    ...
    def scroll_to_value(self, value: float) -> Self:
        return self.scroll_to_row(value)
