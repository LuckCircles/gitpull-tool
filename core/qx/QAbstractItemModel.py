...
class QAbstractItemModel(QObject):
    def __init__(self, **kwargs):
        ...
        self.__ev_layout_changed = QEvent0(
            q_abstract_item_model.layoutChanged
        ).dispose_with(self)
        self.__ev_data_changed = QEvent2[qt.QModelIndex, qt.QModelIndex](
            q_abstract_item_model.dataChanged
        ).dispose_with(self)

        if isinstance(q_abstract_item_model, _QAbstractItemModelImpl):
            self._flags_wrap = QFuncWrap(
                q_abstract_item_model,
                "flags",
                lambda *args, **kwargs: self.flags(*args, **kwargs),
            ).dispose_with(self)

            self._rowCount_wrap = QFuncWrap(
                q_abstract_item_model,
                "rowCount",
                lambda *args, **kwargs: self.row_count(*args, **kwargs),
            ).dispose_with(self)
            self._columnCount_wrap = QFuncWrap(
                q_abstract_item_model,
                "columnCount",
                lambda *args, **kwargs: self.column_count(*args, **kwargs),
            ).dispose_with(self)
            self._index_wrap = QFuncWrap(
                q_abstract_item_model,
                "index",
                lambda *args, **kwargs: self.index(*args, **kwargs),
            ).dispose_with(self)
            self._header_data_wrap = QFuncWrap(
                q_abstract_item_model,
                "headerData",
                lambda *args, **kwargs: self.header_data(*args, **kwargs),
            ).dispose_with(self)

            self._data_wrap = QFuncWrap(
                q_abstract_item_model,
                "data",
                lambda *args, **kwargs: self.data(*args, **kwargs),
            ).dispose_with(self)
            self._parent_wrap = QFuncWrap(
                q_abstract_item_model,
                "parent",
                lambda *args, **kwargs: self._on_parent_wrap(*args, **kwargs),
            ).dispose_with(self)

    @property
    def q_abstract_item_model(self) -> qt.QAbstractItemModel:
