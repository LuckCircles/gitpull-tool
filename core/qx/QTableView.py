...
class QTableView(QWidget):
    def __init__(self, **kwargs):
        super().__init__(
            q_widget=q_init("q_table_view", qt.QTableView, **kwargs), **kwargs
        )

        self._horizontal_header = QHeaderView(
            q_header_view=self.q_table_view.horizontalHeader(), wrap_mode=True
        ).dispose_with(self)

    @property
    def q_table_view(self) -> qt.QTableView:
