...
class QAppWindow(QMainWIndow):
    def __init__(self):
        ...
        (
            self.q_menu_bar.add(
                ...
                .add(
                    QMenu()
                    .set_title("@(Process_priority)")
                    .inline(
                        lambda menu: menu.mx_about_to_show.listen(
                            lambda me=menu: me.dispose_actions()
                            .add(
                                QAction()
                                .set_text(
                                    f"{'-> ' if app.mx_process_priority.get() == ProcessPriority.NORMAL else ''}@(Process_priority.Normal)"
                                )
                                .inline(
                                    lambda act: act.mx_triggered.listen(
                                        lambda: app.mx_process_priority.set(
                            ...
                            .add(
                                QAction()
                                .set_text(
                                    f"{'-> ' if app.mx_process_priority.get() == ProcessPriority.BELOW_NORMAL else ''}@(Process_priority.Low)"
                                )
                                .inline(
                                    lambda act: act.mx_triggered.listen(
                                        lambda: app.mx_process_priority.set(
                ...
                .add(
                    ...
                    .set_text("@(Reset_UI)")
                    .inline(
                        lambda act: act.mx_triggered.listen(lambda: app.reset_state())
                    )
                )
                .add(
                    QAction()
                    .set_text("@(AsyncX_monitor)")
                    .inline(
                        lambda act: act.mx_triggered.listen(
                            lambda: self._on_open_ax_monitor()
                        )
                    )
                )
                .add(
                    QAction()
                    .set_text("@(Show_hide_console)")
                    .inline(
                        lambda act: act.mx_triggered.listen(
                            lambda: QApplication.instance().switch_console_window()
                        )
                    )
                )
                .add(
                    QAction()
                    .set_text("@(Quit)")
                    .inline(lambda act: act.mx_triggered.listen(lambda: app.quit()))
                )
            )
            .add(
                QMenu()
                ...
                .inline(
                    lambda menu: [
                        menu.add(
                            ...
                            .inline(
                                lambda act: act.mx_triggered.listen(
                                    lambda lang=lang: app.mx_language.set(lang)
                                )
                            )
                        )
    ...
    def _on_open_ax_monitor(self):
        if (wnd := getattr(self, "_ax_monitor_wnd", None)) is not None:
            ...
            mx.CallOnDispose(
                lambda: setattr(self, "_ax_monitor_wnd", None)
            ).dispose_with(wnd)
    ...
    def _on_open_about(self):
        if (wnd := getattr(self, "_about_wnd", None)) is not None:
            ...
            wnd.set_parent(self)
            wnd.set_window_icon(self.q_window_icon)
            mx.CallOnDispose(lambda: setattr(self, "_about_wnd", None)).dispose_with(
                wnd
            )
            wnd.mx_close.listen(lambda ev: wnd.dispose())
            wnd.show()
