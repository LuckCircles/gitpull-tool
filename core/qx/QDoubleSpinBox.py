...
class QDoubleSpinBox(QAbstractSpinBox):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            q_abstract_spin_box=q_init(
                "q_double_spin_box", qt.QDoubleSpinBox, **kwargs
            ),
            **kwargs
        )
        ...
        self._mx_value = mx.GetSetProperty[float](
            self.get_value,
            self.set_value,
            QEvent1[float](q_double_spin_box.valueChanged).dispose_with(self),
        ).dispose_with(self)
        self._moving = False
    ...
    @property
    def mx_value(self) -> mx.IProperty_v[float]:
        return self._mx_value

    def get_q_double_spin_box(self) -> qt.QDoubleSpinBox:
        return self.q_abstract_spin_box

    def get_value(self) -> float:
        return self.get_q_double_spin_box().value()
    ...
    def _resize_event(self, ev: qt.QResizeEvent):
        ...
        up_rect = style.subControlRect(
            qt.QStyle.ComplexControl.CC_SpinBox, opt, qt.QStyle.SubControl.SC_SpinBoxUp
        )
        down_rect = style.subControlRect(
            qt.QStyle.ComplexControl.CC_SpinBox,
            opt,
            qt.QStyle.SubControl.SC_SpinBoxDown,
        )
