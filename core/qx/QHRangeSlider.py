...
class QHRangeSlider(QWidget):
    def __init__(self):
        ...
        self.__mx_values = mx.Property[Tuple[int, int]](
            (0, 10), filter=self._flt_values
        ).dispose_with(self)
    ...
    def _size_hint(self) -> qt.QSize:
        ...
        opt = self._get_opt()
        w = 84
        h = self.q_style.pixelMetric(
            qt.QStyle.PixelMetric.PM_SliderThickness, opt, self.q_widget
        )
        ...
        if not self.__read_only:
            ...
            is_first_slider = (
                style.hitTestComplexControl(
                    qt.QStyle.ComplexControl.CC_Slider, opt, pos, self.q_widget
                )
                == qt.QStyle.SubControl.SC_SliderHandle
            )
            ...
            if is_first_slider and is_second_slider:
                is_first_slider = (
                    ...
                    < style.subControlRect(
                        qt.QStyle.ComplexControl.CC_Slider,
                        opt,
                        qt.QStyle.SubControl.SC_SliderHandle,
                    )
                    .center()
            ...
            self.__moving_slider_id = (
                1 if is_first_slider else 2 if is_second_slider else 0
            )
    ...
    def _pixelPosToRangeValue(self, pos: int):
        ...
        gr = style.subControlRect(
            qt.QStyle.ComplexControl.CC_Slider,
            opt,
            qt.QStyle.SubControl.SC_SliderGroove,
        )
        sr = style.subControlRect(
            qt.QStyle.ComplexControl.CC_Slider,
            opt,
        ...
        return style.sliderValueFromPosition(
            self.__minimum, self.__maximum, pos - sliderMin, sliderMax - sliderMin
        )
    ...
    def _paint_event(self, ev: qt.QPaintEvent):
        ...
        opt.subControls = (
            qt.QStyle.SubControl.SC_SliderGroove
            | qt.QStyle.SubControl.SC_SliderTickmarks
        )
