    ...
    def __init__(self, path: mx.IPath_v):
        ...
        (
            self.add(
                QPushButton()
                .h_compact()
                .set_icon(IconDB.close_outline)
                .set_tooltip("@(Close)")
                .inline(
                    lambda btn: btn.mx_clicked.listen(
                        lambda: path.close()
                    ).dispose_with(self)
                )
            )
            .add(
                QPushButton()
                .h_compact()
                .set_icon(IconDB.open_outline)
                .set_tooltip("@(Open)")
                .inline(
                    lambda btn: btn.mx_clicked.listen(
                        lambda: self._open_dlg(QMxPathState._DlgMode.Open)
                    ).dispose_with(self)
                )
            ...
            .add(
                QPushButton()
                .h_compact()
                .set_icon(IconDB.add_circle_outline)
                .set_tooltip("@(New)")
                .inline(
                    lambda btn: btn.mx_clicked.listen(
                        lambda: self._open_dlg(QMxPathState._DlgMode.New)
                    ).dispose_with(self)
                )
                if config.allow_new
                else None
            ...
            .add(
                rename_btn := (
                    (
                        ...
                        .hide()
                        .h_compact()
                        .set_icon(IconDB.pencil_outline)
                        .set_tooltip("@(Change)")
                        .inline(
                            lambda btn: btn.mx_clicked.listen(
                                lambda: self._open_dlg(QMxPathState._DlgMode.Rename)
                            ).dispose_with(self)
                        )
                    )
            ...
            .add(
                reveal_btn := QRevealInExplorerButton(
                    lambda: (
                        (p if p.is_dir() else p.parent)
                        if (p := path.get()) is not None
                        else None
                    )
                )
            )
        ...
        path.mx_opened.reflect(
            lambda opened, enter, bag=mx.Disposable().dispose_with(
                self
            ): self._ref_opened(opened, enter, bag)
        ).dispose_with(self)
    ...
    def _ref_opened(self, opened: bool, enter: bool, bag: mx.Disposable):
        ...
        if enter:
            if opened:
                ...
                self._path.reflect(
                    lambda path: self._line_edit.set_text(str(path))
                ).dispose_with(bag)
    ...
    def _open_dlg(self, mode: QMxPathState._DlgMode):
        ...
        if config.dir:
            ...
            if (extensions := config.extensions) is not None:
                ...
                file_dlg.set_filter(
                    f"{desc} ({' '.join([f'*{ext}' for ext in extensions])})"
                )

        if (
            mode in [QMxPathState._DlgMode.New, QMxPathState._DlgMode.Rename]
            and not config.dir
        ):
