...
from typing import (TYPE_CHECKING, Callable, Deque, Dict, Self, Sequence, Type,
                    TypeVar)
...
class QObject(mx.Disposable):
    ...
    def __init__(self, **kwargs):
        ...
        if (
            self._visible_to_parent_event.__func__ != QObject._visible_to_parent_event
            or self._hiding_from_parent_event.__func__
            != QObject._hiding_from_parent_event
        ):
            # _visible_to_parent_event-related methods are overriden, subscribe self
            self.__vtp_listeners.add(self)

        self.__eventFilter_wrap = QFuncWrap(
            q_object,
            "eventFilter",
            lambda *args, **kwargs: self._event_filter(*args, **kwargs),
        ).dispose_with(self)

        self.__event_wrap = QFuncWrap(
            q_object, "event", lambda *args, **kwargs: self._event(*args, **kwargs)
        ).dispose_with(self)
    ...
    @property
    def q_object(self) -> qt.QObject:
        return self.__q_object

    @property
    def childs(self) -> Sequence[QObject]:
        return reversed(tuple(self.__childs))
    ...
    def set_object_name(self, name: str | None):
        ...
        self.__object_name = (
            f"{self.__class__.__qualname__}{ ('_'+name) if name is not None else ''}"
        )
        return self
    ...
    def __str__(self):
        return f"{super().__str__()}[{self.tree_name}][Childs:{len(self.__childs)}]"  #
