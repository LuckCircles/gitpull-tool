...
class QAxMonitorWindow(QWindow):
    def __init__(self):
        ...
        (
            self.add(
                QLabel().set_text("Active tasks").set_align(Align.CenterF).v_compact()
            ).add(table_view)
        )
...
class QTaskTableModel(QAbstractItemModel):
    def __init__(self, **kwargs):
        ...
        self._layout_upd_timer = (
            QTimer(on_timeout=self._on_layout_upd_timer)
            .dispose_with(self)
            .set_interval(100)
            .start()
        )
        self._upd_timer = (
            QTimer(on_timeout=self._on_upd_timer)
            .dispose_with(self)
            .set_interval(1000)
            .start()
        )
    ...
    def _on_upd_timer(self, *_):
        if len(self._tasks) != 0:
            self._ev_data_changed.emit(
                self.index(0, 0, qt.QModelIndex()),
                self.index(len(self._tasks) - 1, 1, qt.QModelIndex()),
            )
