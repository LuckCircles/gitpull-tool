...
class QShortcut(QObject):
    ...
    @property
    def mx_press(self) -> mx.IEvent0_rv:
        return self._mx_press

    @property
    def mx_release(self) -> mx.IEvent0_rv:
        return self._mx_release

    @property
    def key_comb(self) -> qt.QKeyCombination:
    ...
    def _visible_to_parent_event(self, parent: QObject):
        ...
        if isinstance(parent, QWidget):
            ...
            if self._anchor is None:
                ...
                try:
                    anchor.mx_hide.listen(lambda *_: self.release()).dispose_with(
                        self._anchor_disp_bag
                    )
                except:
                    import code
        ...
        if isinstance(parent, QWindow):
            parent.mx_window_key_press.listen(self._on_window_key_press).dispose_with(
                self._window_disp_bag
            )
            parent.mx_window_key_release.listen(
                self._on_window_key_release
            ).dispose_with(self._window_disp_bag)
            parent.mx_window_leave.listen(lambda: self.release()).dispose_with(
                self._window_disp_bag
            )

        elif isinstance(parent, QApplication):
            parent.mx_focus_widget.reflect(self._on_app_mx_focus_widget).dispose_with(
                self._app_disp_bag
            )
    ...
    def _on_window_key_press(self, ev: qt.QKeyEvent):
        ...
        if self._anchor is not None and not self._typing_focused:
            if not ev.isAutoRepeat():
                ...
                if ev.key() in func_keys:
                    ...
                    key_comb = qt.QKeyCombination(
                        ev.modifiers(), qt.Qt.Key(ev.nativeVirtualKey())
                    )

                if (
                    key_comb.key()
                    in [qt.Qt.Key.Key_Control, qt.Qt.Key.Key_Shift, qt.Qt.Key.Key_Alt]
                    and self._key_comb.key() == key_comb.key()
                ) or self._key_comb == key_comb:
                    self.press()
