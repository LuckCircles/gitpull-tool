...
def patch_dropout_mask(
    ...
    x = (
        ...
        .binomial(1, prob, size=(v_patch_count, h_patch_count, 1))
        .astype(np.uint8)
    )
    x *= 255
...
def clouds(W, H=None, scales=[64, 32, 16, 8]) -> FImage:
    ...
    mod = 1.0
    for scale in scales:
        img = img + noise(W // scale, H // scale).resize(
            W, H, interp=FImage.Interp.CUBIC
        ).apply(lambda img: img * mod)
        mod /= 2.0
...
def binary_stripes(
    W, H=None, line_width=2, density=0.5, scales=[64, 32, 16, 8]
) -> FImage:
...
def c_cut_edges_mask(
    img: cc.c_float32_p,
    W: cc.c_uint32,
    H: cc.c_uint32,
    angle_deg: cc.c_float32,
    edge_dist: cc.c_float32,
    cx: cc.c_float32 = 0.5,
    cy: cc.c_float32 = 0.5,
    cw: cc.c_float32 = 0.5,
    ch: cc.c_float32 = 0.5,
...
def cut_edges_mask(
    ...
    H, W, C = in_out.shape
    c_cut_edges_mask(
        in_out.ctypes.data_as(cc.c_float32_p),
        W,
        H,
        angle_deg,
        edge_dist,
...
def c_icon_loading(
    ...
    H: cc.c_uint32,
    C: cc.c_uint32,
    R_inner: cc.c_float32,
    R_outter: cc.c_float32,
    edge_smooth: cc.c_float32,
    bg_color: cc.c_float32_p,
    fg_color: cc.c_float32_p,
    u_time: cc.c_float32,
) -> cc.c_void: ...
...
def icon_loading(
    ...
    R_outter: float,
    edge_smooth: float,
    bg_color: Sequence[float],
    fg_color: Sequence[float],
    u_time: float,
) -> FImage:
    """H,W,4, float32"""
    ...
    c_icon_loading(
        ...
        R_outter,
        edge_smooth,
        bg_color.ctypes.data_as(cc.c_float32_p),
        fg_color.ctypes.data_as(cc.c_float32_p),
        u_time,
    )
    return FImage.from_numpy(img)
