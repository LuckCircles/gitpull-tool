...
class FImage:
    ...
    def from_state(state: FDict | None) -> FImage | None:
        state = FDict(state)
        if (
            (dtype := state.get("dtype", None)) is not None
            and (shape := state.get("shape", None)) is not None
            and (data := state.get("data", None)) is not None
            and (compressed := state.get("compressed", None)) is not None
        ):
            if compressed:
                return FImage.from_numpy(
                    cv2.imdecode(np.asarray(data, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
                )
            else:
                return FImage.from_numpy(
                    np.frombuffer(data, dtype=np.dtype(dtype)).reshape(shape)
                )
        return None
    ...
    def from_numpy(img: np.ndarray, channels_last=True):
        ...
        if np.issubdtype(img.dtype, np.floating):
            ...
            raise ValueError(
                f"img.dtype must be np.uint8 or np.floating, but passed {img.dtype}."
            )
    ...
    def get_state(self, compressed=True) -> FDict:
        ...
        return FDict(
            {"compressed": c, "data": data, "shape": img.shape, "dtype": img.dtype.name}
        )
    ...
    def ch1(self) -> FImage:
        ...
        self._img = np.dot(
            img[..., 0:3], np.array([0.1140, 0.5870, 0.299], np.float32)
        )[..., None].astype(img.dtype, copy=False)
        return self
    ...
    def bgra(self) -> FImage:
        ...
        if C == 3:
            img = np.pad(
                ...
                ((0, 0), (0, 0), (0, 1)),
                mode="constant",
                constant_values=255 if img.dtype == np.uint8 else 1.0,
            )
    ...
    def f32(self) -> FImage:
        ...
        if dtype == np.uint8:
            ...
            c_u8_to_f32(
                img_out.ctypes.data_as(cc.c_float32_p),
                img.ctypes.data_as(cc.c_uint8_p),
                np.prod(img.shape),
            )
            self = self.__class__.__new__(self.__class__)
            self._img = img_out
    ...
    def u8(self) -> FImage:
        ...
        if img.dtype == np.float32:
            ...
            c_f32_to_u8(
                img_out.ctypes.data_as(cc.c_uint8_p),
                img.ctypes.data_as(cc.c_float32_p),
                np.prod(img.shape),
            )
    ...
    def satushift(self) -> FImage:
        ...
        H, W, C = img.shape
        args = (
            img_out.ctypes.data_as(cc.c_void_p),
    ...
    def blend(self, other: FImage, mask: FImage, alpha=1.0) -> FImage:
        ...
        args = (
            ...
            *img_out.shape,
            img.ctypes.data_as(cc.c_void_p),
            other.ctypes.data_as(cc.c_float32_p),
            *other.shape,
            mask.ctypes.data_as(cc.c_float32_p),
            *mask.shape,
            alpha,
        )
    ...
    def channel_exposure(self, exposure) -> FImage:
        ...
        args = (
            ...
            W,
            C,
            exposure.ctypes.data_as(cc.c_float32_p),
        )
        if img.dtype == np.uint8:
    ...
    def histogram(self, normalized=False) -> np.ndarray:
        ...
        if normalized:
            ...
            c_histogram_u8_f32(
                hist_out.ctypes.data_as(cc.c_float32_p),
                img.ctypes.data_as(cc.c_uint8_p),
                H * W,
                C,
            )
            ...
            c_histogram_u8_uint32(
                hist_out.ctypes.data_as(cc.c_uint32_p),
                img.ctypes.data_as(cc.c_uint8_p),
                H * W,
                C,
            )
    ...
    def levels(self, in_b, in_w, in_g, out_b, out_w) -> FImage:
        ...
        if img.dtype == np.uint8:
            c_levels_u8(
                ...
                W,
                C,
                in_b.ctypes.data_as(cc.c_float32_p),
                in_w.ctypes.data_as(cc.c_float32_p),
                in_g.ctypes.data_as(cc.c_float32_p),
                out_b.ctypes.data_as(cc.c_float32_p),
                out_w.ctypes.data_as(cc.c_float32_p),
            )
        else:
            c_levels_f32(
                img_out.ctypes.data_as(cc.c_float32_p),
    ...
    def motion_blur(self, kernel_size: int, angle: float):
        ...
        self = self.__class__.__new__(self.__class__)
        self._img = cv2.filter2D(
            img, -1, _motion_blur_kernel(kernel_size, angle)
        ).reshape(H, W, C)
    ...
    def pad(
        self, PADL: int = 0, PADT: int = None, PADR: int = None, PADB: int = None
    ) -> FImage:
        """```
        pad with zeros
        ```"""
    ...
    def resize(
        self, OW: int, OH: int, interp: Interp = Interp.LINEAR, smooth=False
    ) -> FImage:
        """resize to (OW,OH)"""
        H, W, C = (img := self._img).shape
    ...
    def remap(
        ...
        self._img = cv2.remap(
            ...
            grid,
            None,
            interpolation=_cv_inter[interp],
    ...
    def warp_affine(self, *args, **kwargs) -> Self:
        ...
        self._img = cv2.warpAffine(
            ...
            mat.as_np(),
            (OW, OH),
            flags=_cv_inter[interp],
            borderMode=_cv_border[border],
        ).reshape(OH, OW, C)
        return self

    def save(
        self, path: Path, fmt_type: ImageFormatType | None = None, quality: int = 100
    ) -> Path:
        """```
        ...
        if fmt_type is None:
            ...
            if suffix in [".jpg", ".jpeg"]:
                ...
                fmt_type = (
                    ImageFormatType.JPEG2000_16 if is_f32 else ImageFormatType.JPEG2000
                )
            elif suffix in [".png"]:
    ...
    def get_perc_hash(self) -> Hash64:
        ...
        img = (
            self.ch1()
            .resize(hash_size * highfreq_factor, hash_size * highfreq_factor)
            .f32()
            .HW()
        )
        ...
        bits = (
            (dct_low_freq > np.median(dct_low_freq))
    ...
    def __add__(self, other) -> Self:
        if isinstance(other, FImage):
            ...
            _c_add_func[img.dtype.type][other.dtype.type](
                img_out.ctypes.data_as(cc.c_void_p),
                img.ctypes.data_as(cc.c_void_p),
                other_img.ctypes.data_as(cc.c_void_p),
                H * W * C,
            )
    ...
    def __sub__(self, other) -> Self:
        if isinstance(other, FImage):
            ...
            _c_sub_func[img.dtype.type][other.dtype.type](
                img_out.ctypes.data_as(cc.c_void_p),
                img.ctypes.data_as(cc.c_void_p),
                other_img.ctypes.data_as(cc.c_void_p),
                H * W * C,
    ...
    def __mul__(self, other) -> Self:
        if isinstance(other, FImage):
            ...
            _c_mul_func[img.dtype.type][other.dtype.type](
                img_out.ctypes.data_as(cc.c_void_p),
                img.ctypes.data_as(cc.c_void_p),
                other_img.ctypes.data_as(cc.c_void_p),
                H * W * C,
    ...
    def __truediv__(self, value) -> Self:
        raise Exception("divide operation with FImage is not allowed. Use .apply()")

    def __rtruediv__(self, value) -> Self:
        raise Exception("divide operation with FImage is not allowed. Use .apply()")

    def __iadd__(self, value) -> Self:
        raise Exception("in-place operation with FImage is not allowed")
    ...
    def __str__(self):
        return f"FImage {self._img.shape} {self._img.dtype}"


_cv_inter = {
    FImage.Interp.NEAREST: cv2.INTER_NEAREST,
...
_cv_border = {
    FImage.Border.CONSTANT: cv2.BORDER_CONSTANT,
...
def c_mul_u8_u8(
    O: cc.c_void_p, A: cc.c_void_p, B: cc.c_void_p, size: cc.c_uint32
) -> cc.c_void: ...
@cc.lib_import(lib_path)
def c_mul_u8_f32(
    O: cc.c_void_p, A: cc.c_void_p, B: cc.c_void_p, size: cc.c_uint32
...
_c_mul_func = {
    np.uint8: {
        np.uint8: c_mul_u8_u8,
        np.float32: c_mul_u8_f32,
    },
    np.float32: {np.uint8: c_mul_f32_u8, np.float32: c_mul_f32_f32},
}
_c_sub_func = {
    np.uint8: {
        np.uint8: c_sub_u8_u8,
        np.float32: c_sub_u8_f32,
    },
    np.float32: {np.uint8: c_sub_f32_u8, np.float32: c_sub_f32_f32},
}
_c_add_func = {
    np.uint8: {
        np.uint8: c_add_u8_u8,
        np.float32: c_add_u8_f32,
    },
...
@cc.lib_import(lib_path)
def c_u8_to_f32(
    img_out: cc.c_float32_p, img_in: cc.c_uint8_p, size: cc.c_int32
) -> cc.c_void: ...
@cc.lib_import(lib_path)
def c_f32_to_u8(
    img_out: cc.c_uint8_p, img_in: cc.c_float32_p, size: cc.c_int32
) -> cc.c_void: ...
@cc.lib_import(lib_path)
def c_histogram_u8_uint32(
    hist_out: cc.c_uint32_p, img_in: cc.c_uint8_p, HW: cc.c_int32, C: cc.c_int32
) -> cc.c_void: ...
@cc.lib_import(lib_path)
def c_histogram_u8_f32(
    hist_out: cc.c_float32_p, img_in: cc.c_uint8_p, HW: cc.c_int32, C: cc.c_int32
) -> cc.c_void: ...


@cc.lib_import(lib_path)
def c_channel_exposure_u8(
    img_out: cc.c_void_p,
    img_in: cc.c_void_p,
    H: cc.c_uint32,
    W: cc.c_uint32,
    C: cc.c_uint32,
    exposure: cc.c_float32_p,
) -> cc.c_void: ...
@cc.lib_import(lib_path)
def c_channel_exposure_f32(
    img_out: cc.c_void_p,
    img_in: cc.c_void_p,
    H: cc.c_uint32,
    W: cc.c_uint32,
...
def c_levels_u8(
    ...
    in_w: cc.c_float32_p,
    in_g: cc.c_float32_p,
    out_b: cc.c_float32_p,
    out_w: cc.c_float32_p,
) -> cc.c_void: ...
@cc.lib_import(lib_path)
def c_levels_f32(
    img_out: cc.c_float32_p,
    img_in: cc.c_float32_p,
    H: cc.c_uint32,
    W: cc.c_uint32,
    C: cc.c_uint32,
    in_b: cc.c_float32_p,
...
def c_hsv_shift_u8(
    ...
    W: cc.c_uint32,
    C: cc.c_uint32,
    h_offset: cc.c_float32,
    s_offset: cc.c_float32,
    v_offset: cc.c_float32,
) -> cc.c_void: ...
@cc.lib_import(lib_path)
def c_hsv_shift_f32(
    O: cc.c_float32_p,
...
def setup_compile():
    ...
    return (
        cc.Compiler(Path(__file__).parent)
        .shared_library(lib_path)
        .minimal_c()
        .optimize()
        .include_glsl()
        .include("FImage.cpp")
        .include("FImage_ispc.o")
        .compile()
    )
