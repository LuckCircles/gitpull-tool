...
def patch_dropout_mask(
    ...
    return lib_gen.patch_dropout_mask(
        W,
        H,
        h_patch_count=rnd_state.randint(
            h_patch_count_range[0], h_patch_count_range[1] + 1
        ),
        v_patch_count=rnd_state.randint(
            v_patch_count_range[0], v_patch_count_range[1] + 1
        ),
        prob=prob,
...
def cut_edges_mask(W: int, H: int = None, cuts_range=[1, 4], seed=None) -> FImage:
    ...
    for i in range(rnd_state.randint(cuts_range[0], cuts_range[1] + 1)):
        lib_gen.cut_edges_mask(
            img,
            angle_deg=rnd_state.uniform() * 360,
            edge_dist=rnd_state.uniform(),
            init=i == 0,
        )
    return FImage.from_numpy(img)


def circle_faded_mask(
    W, H=None, cx_range=[-1.0, 2.0], cy_range=[-1.0, 2.0], f_range=[0.0, 1.5], seed=None
) -> FImage:
    if H is None:
        H = W
    HW_max = max(H, W)
    ...
    return lib_gen.circle_faded(
        W,
        H,
        rnd_state.uniform(cx_range[0] * W, cx_range[1] * W),
...
def noise_clouds(W, H=None) -> FImage:
    ...
    while True:
        ...
        if np.random.randint(2) == 0:
            ...
            x = lib_gen.noise(W // d, H // d).resize(
                W,
                H,
                interp=(
                    FImage.Interp.CUBIC
                    if np.random.randint(2) == 0
                    else FImage.Interp.LANCZOS4
                ),
            )
            if np.random.randint(2) == 0:
                img = img * x
...
def binary_stripes(
    W, H=None, line_width_range=[2, 8], density_range=[0.33, 0.66]
) -> FImage:
    return lib_gen.binary_stripes(
        W,
        H,
        line_width=nprnd.randint(line_width_range[0], line_width_range[1] + 1),
        density=np.random.uniform(*density_range),
    )


def rgb_exposure(
    img: FImage, r_exposure_var=1.0, g_exposure_var=1.0, b_exposure_var=1.0
):
    C = img.shape[-1]
    if C != 3:
...
def channels_deform(img: FImage, alpha=0.25, seed: int | None = None) -> FImage:
    ...
    if img.shape[-1] == 3:
        x = FImage.from_b_g_r(
            Geo(seed=rnd_state.randint(2**31)).transform(
                img.ch1_from_b(), deform_intensity=1.0
            ),
            Geo(seed=rnd_state.randint(2**31)).transform(
                img.ch1_from_g(), deform_intensity=1.0
            ),
            Geo(seed=rnd_state.randint(2**31)).transform(
                img.ch1_from_r(), deform_intensity=1.0
            ),
        )
    else:
        x = Geo(seed=rnd_state.randint(2**31)).transform(
            img.ch1(), deform_intensity=1.0
        )
...
def levels(
    ...
    in_w_range=[0.75, 1.0],
    in_g_range=[0.8, 1.25],
    out_b_range=[0.0, 0.25],
    out_w_range=[0.75, 1.0],
    seed: int | None = None,
) -> FImage:
    C = img.shape[-1]
    rnd_state = np.random.RandomState(seed)
    return img.levels(
        in_b=([rnd_state.uniform(*in_b_range) for _ in range(C)]),
...
def hsv_shift(
    img: FImage,
    h_offset_range=[-1.0, 1.0],
    s_offset_range=[-0.25, 0.25],
    v_offset_range=[-0.25, 0.25],
    seed: int | None = None,
) -> FImage:
    rnd_state = np.random.RandomState(seed)
    return img.hsv_shift(
        rnd_state.uniform(*h_offset_range),
        rnd_state.uniform(*s_offset_range),
        rnd_state.uniform(*v_offset_range),
    )


def box_sharpen(
    img: FImage, kernel_range=None, power_range=[0.0, 2.0], seed: int | None = None
) -> FImage:
    if kernel_range is None:
        kernel_range = [1, max(1, max(img.shape[0:2]) // 32)]

    rnd_state = np.random.RandomState(seed)
    return img.box_sharpen(
        kernel_size=int(rnd_state.uniform(*kernel_range)),
        power=rnd_state.uniform(*power_range),
    )


def gaussian_sharpen(
    img: FImage, sigma_range=None, power_range=[0.0, 2.0], seed: int | None = None
) -> FImage:
    if sigma_range is None:
        sigma_range = [0, max(img.shape[0:2]) / 32.0]
    rnd_state = np.random.RandomState(seed)
    return img.gaussian_sharpen(
        sigma=rnd_state.uniform(*sigma_range), power=rnd_state.uniform(*power_range)
    )
...
def median_blur(img: FImage, kernel_range=None, seed: int | None = None) -> FImage:
    if kernel_range is None:
        kernel_range = [1, max(1, max(img.shape[0:2]) // 32)]
    rnd_state = np.random.RandomState(seed)
    return img.median_blur(
        kernel_size=rnd_state.randint(kernel_range[0], kernel_range[1] + 1)
    )


def motion_blur(
    img: FImage, kernel_range=None, angle_range=[0, 360], seed: int | None = None
) -> FImage:
    if kernel_range is None:
        kernel_range = [1, max(1, max(img.shape[0:2]) // 8)]
    rnd_state = np.random.RandomState(seed)
    return img.motion_blur(
        kernel_size=rnd_state.randint(kernel_range[0], kernel_range[1] + 1),
        angle=rnd_state.randint(*angle_range),
    )
...
def jpeg_artifacts(
    img: FImage, quality_range=[10, 100], seed: int | None = None
) -> FImage:
    """
    quality    0-100
    """
