...
class TransformParams:
    ...
    def copy(self) -> TransformParams:
        return TransformParams(
            tx=self._tx, ty=self._ty, scale=self._scale, rot_deg=self._rot_deg
        )
    ...
    def generate(
        ...
        return TransformParams(
            tx=rnd_state.uniform(-tx_var, tx_var),
    ...
    def __add__(self, value: TransformParams) -> TransformParams:
        return self.added(
            tx=value._tx, ty=value._ty, scale=value._scale, rot_deg=value._rot_deg
        )
...
class Geo:
    ...
    def __init__(
        self,
        offset_transform_params: TransformParams | None = None,
        ...
        if deform_transform_params is None:
            deform_transform_params = TransformParams.generate(
                tx_var=0.0,
                ty_var=0.0,
                scale_var=0.0,
                rot_deg_var=0.0,
    ...
    def transform(
        ...
        remap_grid, mask = self._gen_remap_grid(
            W,
            H,
            OW,
            OH,
            center_fit=center_fit,
            tr_params=self._offset_transform_params
            + self._transform_params.scaled(transform_intensity),
    ...
    def _get_cached(
        ...
        s2t_mat = (
            FAffMat2()
            .translate((-0.5 + tr_params._tx) * SW, (-0.5 + tr_params._ty) * SH)
        ...
        if deform_intensity != 0.0:
            ...
            t_deform_grid = gen_grid(
                TW, TH, deform_cell_count, deform_intensity, seed=grid_seed
            )  #
            ...
            t2t_deform_mat = (
                FAffMat2()
                .translate(
                    (-0.5 + deform_tr_params._tx) * TW,
                    (-0.5 + deform_tr_params._ty) * TH,
                )
            ...
            t_remap_grid = cv2.warpAffine(
                s_remap_grid,
                s2t_mat_t2t_deform_mat.as_np(),
                (TW, TH),
            ...
            t_diff_deform_grid = t_remap_grid - cv2.remap(
                t_remap_grid,
                t_deform_grid,
                None,
            ...
            s_diff_deform_grid = cv2.warpAffine(
                t_diff_deform_grid,
                s2t_mat_t2t_deform_mat.inverted.as_np(),
                (SW, SH),
                flags=cv2.INTER_LINEAR,
                borderMode=cv2.BORDER_CONSTANT,
            )
    ...
    def _gen_remap_grid(
        ...
        s2t_mat, s_remap_grid, mask = self._get_cached(
            SW,
            SH,
            TW,
            TH,
        ...
        t_remap_grid = cv2.warpAffine(
            ...
            (TW, TH),
            flags=cv2.INTER_LINEAR,
            borderMode=FImage._border_to_cv(
                FImage.Border.REPLICATE if border == FImage.Border.CONSTANT else border
            ),
        )
...
def c_gen_grid(
    out_grid: cc.c_float32_p,
    OW: cc.c_int32,
    OH: cc.c_int32,
    cell_count: cc.c_int32,
    intensity: cc.c_float32,
    seed: cc.c_uint32,
) -> cc.c_void: ...
def gen_grid(
    W: int, H: int, cell_count: int, intensity: float, seed: int
) -> np.ndarray:
    """bilinear interpolation with align corners."""
    out_grid = np.empty((H, W, 2), np.float32)
    c_gen_grid(
        out_grid.ctypes.data_as(cc.c_float32_p), W, H, cell_count, intensity, seed
    )
    return out_grid
