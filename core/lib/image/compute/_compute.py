...
def find_nearest_hist(
    hist_ar: np.ndarray, c_idx: int, s_idx: int, e_idx: int, c_avg_count=16
) -> int:
    """
    ...
    if hist_ar.dtype == np.int32:
        return c_find_nearest_hist_u8(
            hist_ar.ctypes.data_as(cc.c_uint8_p), S, c_idx, c_avg_count, s_idx, e_idx
        )
    elif hist_ar.dtype == np.float32:
        return c_find_nearest_hist_f32(
            hist_ar.ctypes.data_as(cc.c_float32_p), S, c_idx, c_avg_count, s_idx, e_idx
        )
...
def c_find_nearest_hist_u8(
    hist_ar: cc.c_uint8_p,
    S: cc.c_int32,
    c_idx: cc.c_int32,
    c_avg_count: cc.c_int32,
    s_idx: cc.c_int32,
    e_idx: cc.c_int32,
) -> cc.c_int32: ...
@cc.lib_import(lib_path)
def c_find_nearest_hist_f32(
    hist_ar: cc.c_float32_p,
    S: cc.c_int32,
    c_idx: cc.c_int32,
    c_avg_count: cc.c_int32,
    s_idx: cc.c_int32,
