...
class FEmbedAlignedFaceInfo:
    ...
    def from_embed(path: Path) -> FEmbedAlignedFaceInfo | None:
        ...
        try:
            if (
                data_bytes := sfeas.read(path, FEmbedAlignedFaceInfo._UUID)
            ) is not None:
    ...
    def from_state(state: FDict | None) -> FEmbedAlignedFaceInfo | None:
        ...
        state = FDict(state)
        if (
            (aligned_face := FAlignedFace.from_state(state.get("aligned_face", None)))
            is not None
            and (source_face := FFace.from_state(state.get("source_face", None)))
            is not None
            and (source_frame := FFrame.from_state(state.get("source_frame", None)))
            is not None
        ):
    ...
    def __init__(
        self, aligned_face: FAlignedFace, source_face: FFace, source_frame: FFrame
    ):
    ...
    def get_state(self) -> FDict:
        return FDict(
            {
                "aligned_face": self._aligned_face.get_state(),
    ...
    @property
    def aligned_face(self) -> FAlignedFace:
        return self._aligned_face

    @property
    def source_face(self) -> FFace:
        return self._source_face

    @property
    def source_frame(self) -> FFrame:
    ...
    def set_aligned_face(self, aligned_face: FAlignedFace) -> Self:
        f = self.clone()
        f._aligned_face = aligned_face
        return f

    def set_source_face(self, source_face: FFace) -> Self:
        f = self.clone()
        f._source_face = source_face
        return f

    def set_source_frame(self, source_frame: FFrame) -> Self:
        f = self.clone()
        f._source_frame = source_frame
        return f
