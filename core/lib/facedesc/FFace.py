...
class FFace:
    ...
    def from_state(state: FDict | None) -> FFace | None:
        ...
        if (rect := FRectf.from_state(state.get("rect", None))) is not None:
            ...
            if (
                annotations := FAnnoList.from_state(state.get("annotations", None))
            ) is None:
    ...
    @staticmethod
    def sorted_by_confidence(faces: Sequence[FFace]) -> Sequence[FFace]:
        return sorted(
            faces,
            key=lambda face: face.confidence if face.confidence is not None else 0,
            reverse=True,
        )

    @staticmethod
    def sorted_by_largest(faces: Sequence[FFace]) -> Sequence[FFace]:
        return sorted(faces, key=lambda face: face.rect.area, reverse=True)

    @staticmethod
    def sorted_by_dist_from(faces: Sequence[FFace], pt: FVec2f) -> Sequence[FFace]:
        return sorted(faces, key=lambda face: (face.rect.pc - pt).length)

    @staticmethod
    def sorted_by_left_to_right(faces: Sequence[FFace]) -> Sequence[FFace]:
        return sorted(faces, key=lambda face: face.rect.pc.x)

    @staticmethod
    def sorted_by_right_to_left(faces: Sequence[FFace]) -> Sequence[FFace]:
        return sorted(faces, key=lambda face: face.rect.pc.x, reverse=True)

    @staticmethod
    def sorted_by_top_to_bottom(faces: Sequence[FFace]) -> Sequence[FFace]:
        return sorted(faces, key=lambda face: face.rect.pc.y)

    @staticmethod
    def sorted_by_bottom_to_top(faces: Sequence[FFace]) -> Sequence[FFace]:
        return sorted(faces, key=lambda face: face.rect.pc.y, reverse=True)
    ...
    def transform(self, mat: FAffMat2) -> Self:
        ...
        return self.set_rect(self._rect.transform(mat)).set_annotations(
            self._annotations.transform(mat)
        )

    def set_rect(self, rect: FRectf) -> Self:
        f = self.clone()
        f._rect = rect
        return f

    def set_confidence(self, confidence: float | None) -> Self:
        f = self.clone()
        f._confidence = confidence
        return f

    def set_detector_id(self, detector_id: str | None) -> Self:
        f = self.clone()
        f._detector_id = detector_id
        return f

    def set_annotations(self, annotations: FAnnoList) -> Self:
        f = self.clone()
    ...
    def align(
        ...
        if (
            align_lmrks := self.annotations.get_first_by_class(FAnnoLmrk2DYSARange)
        ) is not None:
            align_lmrks = align_lmrks.to_2DYSA(y_axis_offset)
        else:
            align_lmrks = self.annotations.get_first_by_class_prio(
                [FAnnoLmrk2DYSA, FAnnoLmrk2D106, FAnnoLmrk2D68, FAnnoLmrk2D]
            )

        if isinstance(align_lmrks, FAnnoLmrk2D):

            # Get align mat with specified params
            align_mat = align_lmrks.get_align_mat(
                coverage=coverage, ux_offset=ux_offset, uy_offset=uy_offset
            )
