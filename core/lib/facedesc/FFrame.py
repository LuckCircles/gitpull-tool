...
class FFrame:
    ...
    def from_state(state: FDict | None) -> FFrame | None:
        ...
        if (image_size_state := state.get("image_size", None)) is not None and (
            image_size := FVec2i.from_state(image_size_state)
        ) is not None:
    ...
    def set_media_url(self, media_url: str | None) -> Self:
        f = self.clone()
        f._media_url = media_url
        return f

    def set_frame_idx(self, frame_idx: int | None) -> Self:
        f = self.clone()
        f._frame_idx = frame_idx
        return f
