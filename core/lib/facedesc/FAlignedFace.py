...
class FAlignedFace:
    ...
    def from_state(state: FDict | None) -> FAlignedFace | None:
        ...
        if (
            (image_size := FVec2i.from_state(state.get("image_size", None))) is not None
            and (mat := FAffMat2.from_state(state.get("mat", None))) is not None
            and (coverage := state.get("coverage", None)) is not None
        ):
    ...
    def get_state(self) -> FDict:
        return FDict(
            {
                "image_size": self._image_size.get_state(),
    ...
    def transform(self, mat: FAffMat2) -> Self:
        ...
        return self.set_mat(self._mat * mat).set_annotations(
    ...
    def set_image_size(self, image_size: FVec2i) -> Self:
        f = self.clone()
        f._image_size = image_size
        return f

    def set_mat(self, mat: FAffMat2) -> Self:
        f = self.clone()
        f._mat = mat
        return f

    def set_coverage(self, coverage: float) -> Self:
        f = self.clone()
        f._coverage = coverage
        return f
