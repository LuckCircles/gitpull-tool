...
class VideoSource(mx.Disposable):
    ...
    def open(path: Path | str, fps=None) -> VideoSource:
        ...
        if err is None:
            ...
            if any(
                x is None
                for x in [v_idx, v_width, v_height, v_start_time, v_duration, v_fps]
            ):  #
                err = "Wrong metadata"
    ...
    def _ffmpeg_restart(self) -> bool:
        ...
        try:
            args = [  #'-loglevel', 'error', #warning
                "-ss",
                str(self._v_start_time + self._frame / self._v_fps),
                "-i",
                str(self._path),
                "-r",
                str(self._v_fps),
    ...
    def _ffmpeg_next_frame_bytes(self) -> bytes | None:
        if self._ffmpeg_proc is not None and (
            ...
            try:
                buf = self._ffmpeg_proc.stdout.read(
                    self._v_height * self._v_width * self._ch * self._ch_bytes
                )
            except:
                buf = []
    ...
    def _ffmpeg_next_frame(self) -> FImage | None:
        if (buf := self._ffmpeg_next_frame_bytes()) is not None:
            img = np.ndarray(
                (self._v_height, self._v_width, self._ch), dtype=self._dtype, buffer=buf
            )
