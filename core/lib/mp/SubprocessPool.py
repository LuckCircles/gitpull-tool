...
class SubprocessClass(mx.Disposable):
    ...
    @staticmethod
    def _main(
        scls: Type[SubprocessClass],
        process_id,
        prio,
        h2p_q: multiprocessing.Queue,
        p2h_q: multiprocessing.Queue,
        started_ev: multiprocessing.Event,
        closing_ev: multiprocessing.Event,
        closed_ev: multiprocessing.Event,
    ):
        started_ev.set()
        if prio is not None:
        ...
        while not closing_ev.is_set():
            ...
            while not h2p_q.empty():
                ...
                if is_cancel:
                    ...
                    t.attach_to(fg)
                    t.call_on_finish(
                        lambda t, job_id=job_id: (
                            job_tasks.pop(job_id),
                            p2h_q.put(
                                (
                                    self.__process_id,
                                    job_id,
                                    t.succeeded,
                                    t.result if t.succeeded else t.error,
                                )
                            ),
                        )
                    )
...
class SubprocessPool(mx.Disposable):
    ...
    class SubprocessDeadException(Exception):
        def __init__(self):
            super().__init__("Subprocess is dead.")

    def __init__(
        self,
        scls: SubprocessClass,
        ...
        self._process_count = process_count = max(
            1, int(multiprocessing.cpu_count() * process_frac)
        )
    ...
    def _bg_task(self):
        ...
        while True:
            while not self._q_p2h.empty():
                ...
                if (
                    fut := job_result_futures[process_id].get(job_id, None)
                ) is not None:
                    if job_succeeded:
    ...
    def process(self, *args, **kwargs):
        ...
        try:
            ...
            while not all(
                (ev := self._evs_started[i]) is not None and ev.is_set()
                for i in range(self._process_count)
            ):
                ax.sleep(0)
            ...
            process_id = sorted(
                [(i, n_jobs_in_processes[i]) for i in range(self._process_count)],
                key=lambda x: x[1],
            )[0][0]
