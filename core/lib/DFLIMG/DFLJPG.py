...
class SegIEPoly:
    ...
    def add(self, x, y):
        self.pts = np.append(
            self.pts[0 : self.n], [(float(x), float(y))], axis=0
        ).astype(np.float32)
    ...
    def insert_pt(self, n, pt):
        ...
        self.pts = np.concatenate(
            (self.pts[0:n], pt[None, ...].astype(np.float32), self.pts[n:]), axis=0
        )
        self.n_max = self.n = self.n + 1
...
class SegIEPolys:
    ...
    def sort(self):
        ...
        self.polys = (
            poly_by_type[SegIEPolyType.INCLUDE] + poly_by_type[SegIEPolyType.EXCLUDE]
        )
    ...
    def overlay_mask(self, mask):
        ...
        for poly in self.polys:
            ...
            if len(pts) != 0:
                cv2.fillPoly(
                    mask, [pts], white if poly.type == SegIEPolyType.INCLUDE else black
                )
...
class DFLJPG(object):
    ...
    def set_xseg_mask(self, mask_a):
        ...
        if not ret or len(buf) > data_max_len:
            for jpeg_quality in range(100, -1, -1):
                ret, buf = cv2.imencode(
                    ".jpg", img_data, [int(cv2.IMWRITE_JPEG_QUALITY), jpeg_quality]
                )
        ...
        if not ret:
            raise Exception(
                "set_xseg_mask: unable to generate image data for set_xseg_mask"
            )
