...
from ..math import polygon_area, rotation_matrix_to_euler, umeyama
...
class imagelib:
    ...
    def draw_polygon(image, points, color, thickness=1):
        ...
        cv2.polylines(image, [pts], True, color, int(thickness), lineType=cv2.LINE_AA)

    @staticmethod
    def normalize_channels(img, channels):
        if len(img.shape) == 2:
            # Already single channel
            return img
        elif len(img.shape) == 3 and img.shape[2] == channels:
            # Already has desired number of channels
            return img
        elif len(img.shape) == 3 and img.shape[2] > channels:
            # Reduce channels
            if channels == 1:
...
def convert_98_to_68(lmrks):
    ...
    result += [lmrks[60], lmrks[61], lmrks[63], lmrks[64], lmrks[65], lmrks[67]]

    # right eye
    result += [lmrks[68], lmrks[69], lmrks[71], lmrks[72], lmrks[73], lmrks[75]]
...
def get_transform_mat(image_landmarks, output_size, face_type, scale=1.0):
    ...
    if not remove_align:
        ...
        area = polygon_area(np.column_stack((l_t[:, 0], l_t[:, 1])))
...
def get_cmask(image_shape, lmrks, eyebrows_expand_mod=1.0):
    ...
    def process(w, h, data):
        ...
        for s, pts_loop_ar in data:
            ...
            for pts, loop in pts_loop_ar:
                ...
                lines.append(
                    [
                        [pts[i], pts[(i + 1) % pts_len]]
                        for i in range(pts_len - (0 if loop else 1))
                    ]
                )
        ...
        all_lines = np.repeat(all_lines[None, ...], pts_count, axis=0).reshape(
            (pts_count * line_count, 2, 2)
        )
        ...
        ba = b - a
        ph = np.clip(
            np.einsum("ij,ij->i", pa, ba) / np.einsum("ij,ij->i", ba, ba), 0, 1
        )
    ...
    nose_mask = blend(
        gdf("up_nose", nose_thickness),
        gdf("down_nose", nose_thickness),
        nose_thickness * 3,
    )
    nose_mask = 1 - np.clip(nose_mask / nose_fall_dist, 0, 1)
...
def get_face_struct_mask(
    image_shape, image_landmarks, eyebrows_expand_mod=1.0, color=(1,)
):
...
def draw_landmarks(
    ...
    cv2.polylines(
        image,
        tuple(np.array([v]) for v in (right_eye, left_eye, mouth)),
        True,
        color,
        thickness=thickness,
        lineType=cv2.LINE_AA,
    ...
    if draw_circles:
        ...
        for x, y in np.concatenate(
            (right_eyebrow, left_eyebrow, mouth, right_eye, left_eye, nose), axis=0
        ):
...
def draw_rect_landmarks(
    ...
    draw_landmarks(
        image, image_landmarks, color=landmarks_color, transparent_mask=transparent_mask
    )
    ...
    points = transform_points(
        [
            (int(face_size * 0.05), 0),
            (int(face_size * 0.1), int(face_size * 0.1)),
            (0, int(face_size * 0.1)),
        ],
        image_to_face_mat,
        True,
...
def calc_face_pitch(landmarks):
    ...
    t = (
        (landmarks[6][1] - landmarks[8][1]) + (landmarks[10][1] - landmarks[8][1])
    ) / 2.0
    b = landmarks[8][1]
    return float(b - t)
...
def estimate_pitch_yaw_roll(aligned_landmarks, size=256):
    ...
    (_, rotation_vector, _) = cv2.solvePnP(
        ...
        np.concatenate(
            (aligned_landmarks[:27], aligned_landmarks[30:36]), axis=0
        ).astype(np.float32),
        camera_matrix,
        np.zeros((4, 1)),
    )

    pitch, yaw, roll = rotation_matrix_to_euler(cv2.Rodrigues(rotation_vector)[0])
...
#    area = polygon_area(np.column_stack((bbox[:,0], bbox[:,1])))
