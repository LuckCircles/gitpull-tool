...
def parents_remained(path: Path, count: int) -> Path:
    ...
    parents = [
        parent.name for parent in reversed(path.parents) if len(parent.name) != 0
    ]
    if len(path.drive) != 0:
...
def get_files_paths(
    dir_path: Path | str, extensions=None, subdirs=False, relative=False
) -> List[Path]:
    """
    ...
    return list(
        gen_files_paths(
            dir_path, extensions=extensions, subdirs=subdirs, relative=relative
        )
    )
...
def gen_dir_paths(
    dir_path: Path | str, subdirs=False, relative=False
) -> Generator[Path, None, None]:
    """
...
def abspath(path: Path, cwd: Path = None) -> Path:
    ...
    if not path.is_absolute():
        path = Path(
            os.path.abspath((cwd if cwd is not None else Path(os.getcwd())) / path)
        )
