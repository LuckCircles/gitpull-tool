...
class IFSIP_v(IFSIP_rv):
    ...
    def delete_items(
        self, indices: FIndices | Iterable | int | None
    ) -> IFSIP_rv.DeleteResult: ...
    def copy_items(
        self,
        indices: FIndices | Iterable | int | None | Mapping[int, str],
        fsip_root: Path,
    ) -> IFSIP_v.CopyResult: ...
    def move_items(
        self, indices: FIndices | Iterable | int | Mapping[int, str], fsip_root: Path
    ) -> IFSIP_rv.MoveResult: ...
...
class FSIPInfo:
    ...
    @property
    def avail_image_suffixes(self) -> Sequence[str]:
        return ImageFormatSuffixes

    def get_pair_dir_path(self, pair_type: str) -> Path:
        return self._root / pair_type.lower()
    ...
    def load_item_rel_paths(self) -> Sequence[Path]:
        ...
        raise on error
        """
        return lib_path.get_files_paths(
            self._root, extensions=self.avail_image_suffixes, relative=True
        )
    ...
    def load_pair_rel_paths(self, pair_type: str) -> Sequence[Path]:
        ...
        """
        return lib_path.get_files_paths(
            self.get_pair_dir_path(pair_type),
            extensions=self.avail_image_suffixes,
            relative=True,
        )
    ...
    def load_pair_types(self) -> Sequence[str]:
        ...
        """
        pair_types = set(
            path.name.lower() for path in lib_path.get_dir_paths(self._root)
        )
...
class FSIP(IFSIP_v):
    ...
    class Pair:
        ...
        def __str__(self):
            return f"[Pair {self._type} {self._suffix}]"
    ...
    class Item:
        ...
        def has_pair(self, pair_type: str) -> bool:
            return self.get_pair(pair_type) is not None
        ...
        def __str__(self):
            return f"[Item {self._name} {self._suffix} {self._pairs}]"
    ...
    def open(root: Path | str) -> FSIP:
        ...
        if len(item_rel_paths) == len(set(x.stem for x in item_rel_paths)):
            ...
            items_data = {
                ...
                for i, image_rel_path in enumerate(item_rel_paths)
            }
            ...
            for pair_type in (pair_types := info.load_pair_types()):
                for pair_rel_path in info.load_pair_rel_paths(pair_type):
                    ...
                    if (
                        item_dict := items_data.get(pair_rel_path.stem, None)
                    ) is not None:
            ...
            for item_name, item_data in items_data.items():
                ...
                items[item_id] = FSIP.Item(
                    item_name,
                    item_data["suffix"],
                    tuple(
                        FSIP.Pair(pair_type, pair_suffix)
            ...
            dups = [
                ...
                for key, group in itertools.groupby(
                    item_rel_paths, key=lambda x: x.stem
                )
                if len(group_list := list(group)) > 1
            ]

            errs.append(
                f"Duplicated items: {';'.join([ str(x) for x in sum(dups, []) ])}"
            )
    ...
    def add_pair_type(self, pair_type: str):
        ...
        if len(pair_type) != 0:
            ...
            if pair_type not in self._pair_types:
                self._info.get_pair_dir_path(pair_type).mkdir(
    ...
    def delete_pair_type(self, pair_type: str):
        ...
        if pair_type in self._pair_types:
            ...
            self._pair_types = tuple(
                self._pair_types[:idx] + self._pair_types[idx + 1 :]
            )
    ...
    def generate_free_names(self, amount: int) -> Sequence[str]:
        ...
        all_item_names = set(
            self.get_item_name(item_id) for item_id in range(self.item_count)
        )
        names = []
        n = 0
        i = 0
    ...
    def has_pair_type(self, pair_type: str) -> bool:
        return pair_type.lower() in self._pair_types

    def has_pair(self, item_id: int, pair_type: str) -> bool:
        return self._items[item_id].has_pair(pair_type)
    ...
    def delete_items(
        ...
        try:
            temp_fsip = FSIPInfo(
                self.root.parent / f"{self.root.name}_{uuid.uuid4().hex}"
            )
        ...
        return FSIP.DeleteResult(
            errors=result.errors,
            deleted_ids=result.moved_ids,
            non_deleted_ids=result.non_moved_ids,
        )
    ...
    def copy_items(
        ...
        for item_id in indices.discard_ge(len(items)):
            ...
            try:
                ...
                copyables = [
                    (
                        self._root / item.name_suffix,
                        info.root / (new_name + item.suffix),
                    )
                ]
                for pair in item.pairs:
                    copyables.append(
                        (
                            self._root / pair.type / (item.name + pair.suffix),
...
class _FSIPFilteredView(IFilteredFSIP_v):
    ...
    @property
    def root(self) -> Path:
        return self._fsip.root

    @property
    def info(self) -> FSIPInfo:
        return self._fsip.info
    ...
    @property
    def pair_types(self) -> Sequence[str]:
        return self._fsip.pair_types

    def add_pair_type(self, pair_type: str):
        self._fsip.add_pair_type(pair_type)

    def add_pair_path(self, item_id: int, pair_type: str, pair_suffix: str) -> Path:
        return self._fsip.add_pair_path(
            self._indices.to_list()[item_id], pair_type, pair_suffix
        )

    def delete_pair(self, item_id: int, pair_type: str):
        return self._fsip.delete_pair(self._indices.to_list()[item_id], pair_type)

    def delete_pair_type(self, pair_type: str):
        self._fsip.delete_pair(pair_type)

    def get_item_path(self, item_id: int) -> Path:
        return self._fsip.get_item_path(self._indices.to_list()[item_id])

    def get_pair_path(self, item_id: int, pair_type: str) -> Path | None:
        return self._fsip.get_pair_path(self._indices.to_list()[item_id], pair_type)

    def has_pair_type(self, pair_type: str) -> bool:
        return self._fsip.has_pair_type(pair_type)

    def has_pair(self, item_id: int, pair_type: str) -> bool:
        return self._fsip.has_pair(self._indices.to_list()[item_id], pair_type)
    ...
    def copy_items(
        ...
        result = self._fsip.copy_items(
            self.to_orig_indices(indices), fsip_root=fsip_root
        )
