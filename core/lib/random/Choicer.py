...
class Choicer:
    ...
    def __init__(
        self, items: Sequence[Any | Choicer], probs: Sequence[int | float] | np.ndarray
    ):
        """probs   [ 0.001 .. 1.0 ]"""
        self._items = items
        ...
        if len(self._items) != 0:
            ...
            self._idxs_base = np.concatenate(
                [np.full((x,), i, dtype=np.uint32) for i, x in enumerate(rates)], 0
            )
    ...
    @property
    def items(self) -> Sequence[Any | Choicer]:
        return self._items
