...
class DeviceRef:
    ...
    def from_state(state: FDict = None) -> DeviceRef:
        ...
        return DeviceRef(
            execution_provider=state.get("execution_provider", "CPUExecutionProvider"),
    ...
    def info(self) -> DeviceInfo:
        if not self.is_cpu:
            for info in get_avail_gpu_devices_info():
                if (
                    info.execution_provider == self._execution_provider
                    and info.index == self._index
                ):
                    return info
        # Fallback to CPU
    ...
    def is_cpu(self) -> bool:
        return self._execution_provider == "CPUExecutionProvider"

    def get_state(self) -> FDict:
        return FDict(
            {"execution_provider": self._execution_provider, "index": self._index}
        )

    def __hash__(self):
        return (self._execution_provider, self._index).__hash__()

    def __eq__(self, other):
        if (
            self is not None
            and other is not None
            and isinstance(self, DeviceRef)
            and isinstance(other, DeviceRef)
        ):
            return (
...
def get_cpu_device() -> DeviceRef:
    return DeviceRef(execution_provider="CPUExecutionProvider", index=-1)


def get_avail_gpu_devices() -> List[DeviceRef]:
    return [
        DeviceRef(execution_provider=dev.execution_provider, index=dev.index)
        for dev in get_avail_gpu_devices_info()
    ]
...
def get_best_device(devices: List[DeviceRef]) -> DeviceRef:
    ...
    gpu_devices = sorted(
        [device for device in devices if not device.is_cpu],
        key=lambda device: device.info.total_memory,
        reverse=True,
    )
...
def _get_ort_devices_info() -> List[DeviceInfo]:
    ...
    if "CUDAExecutionProvider" in prs:  # not lib_appargs.get_arg_bool('NO_CUDA') and
        ...
        try:
            ...
            if cuda.cuInit(0) == 0 and cuda.cuDeviceGetCount(ctypes.byref(nGpus)) == 0:
