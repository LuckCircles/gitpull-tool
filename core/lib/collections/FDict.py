    ...
    def __init__(self, d: HFDict | FDict | Mapping | None = None, **kwargs):
        ...
        if not kwargs.pop("_checked", False):
            if d is None:
                ...
                d = {
                    _checked_key(k): _immutable_repack(v, **kwargs)
                    for k, v in d.items()
                }
            else:
...
class HFDict(MutableMapping):
    ...
    def get(self, key, default): ...
    def get(self, *args):
        return self._f_dict.get(*args)

    def items(self) -> ItemsView:
        return self._f_dict.items()

    def keys(self) -> KeysView:
        return self._f_dict.keys()

    def values(self) -> ValuesView:
        return self._f_dict.values()

    def __getitem__(self, key):
        return self._f_dict.__getitem__(key)

    def __contains__(self, key) -> bool:
        return self._f_dict.__contains__(key)

    # Collection
    def __len__(self) -> int:
        return self._f_dict.__len__()

    # Iterable
    def __iter__(self) -> Iterator:
        return self._f_dict.__iter__()
    ...
    def __repr__(self):
        return self._f_dict.__repr__()


class _FSequence(tuple):
    def __new__(self, iterable, **kwargs) -> _FSequence:
        return tuple.__new__(
            _FSequence, (_immutable_repack(v, **kwargs) for v in iterable)
        )


class _FSet(frozenset):
    def __new__(self, iterable, **kwargs) -> _FSet:
        return frozenset.__new__(
            _FSet, (_immutable_repack(v, **kwargs) for v in iterable)
        )
...
def _mutable_repack(value, **kwargs):
    ...
    if value_type in _base_types:
        ...
        value = {
            _checked_key(k): _mutable_repack(v, **kwargs) for k, v in value.items()
        }
    else:
        raise ValueError(f"Type of value {value_type} is not allowed in state")
