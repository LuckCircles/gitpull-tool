...
class FIndices(Collection):
    ...
    def __init__(self, indices: FIndices | Iterable | int | None = None, **kwargs):
        ...
        if (bits := kwargs.get("_bits", None)) is not None:
            ...
            bits = np.zeros(
                (1 + indices.max() if indices.shape[0] != 0 else 0,), dtype=bool
            )
            bits[indices] = True
    ...
    def discard(
        self, indices: FIndices | Iterable | int | None, shift=False
    ) -> FIndices:
        """
        Same as difference.
    ...
    def indwhere(self, indices: FIndices | Iterable | int | None) -> FIndices:
        ...
        x = np.argwhere(
            np.in1d(np.argwhere(self._bits)[:, 0], np.argwhere(indices._bits)[:, 0])
        )[:, 0]
        return FIndices(x)

    @cached_method
    def to_list(self) -> Sequence:
        return tuple(np.argwhere(self._bits)[:, 0].tolist())

    @cached_method
    def to_set(self) -> frozenset:
        return frozenset(np.argwhere(self._bits)[:, 0])
    ...
    def __or__(self, indices: FIndices | Iterable | int | None) -> FIndices:
        bits1, bits2 = FIndices._broadcast_to_greater(
            self._bits, FIndices(indices)._bits
        )
        return FIndices(_bits=bits1 | bits2)
    ...
    def __repr__(self) -> str:
        return f"FIndices({self.to_list().__repr__()})"
