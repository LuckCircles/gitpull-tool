...
if is_win:
    ...
    PriorityClass_2_kernel32_ProcessPriority = {
        val: key for key, val in kernel32_PriorityClass_2_ProcessPriority.items()
    }
    ...
    ThreadPriority_2_kernel32_ThreadPriority = {
        val: key for key, val in kernel32_ThreadPriority_2_ThreadPriority.items()
    }
...
def get_process_priority() -> ProcessPriority:
    ...
    if is_win:
        return kernel32_PriorityClass_2_ProcessPriority[
    ...
    try:
        if is_win:
            ...
            kernel32.SetPriorityClass(
                hProcess, PriorityClass_2_kernel32_ProcessPriority[prio]
            )
        elif is_linux:
            val = {
...
def get_thread_priority() -> ThreadPriority:
    """ """
    if is_win:
        return kernel32_ThreadPriority_2_ThreadPriority[
            kernel32.GetThreadPriority(kernel32.GetCurrentThread())
        ]
    ...
    try:
        if is_win:
            kernel32.SetThreadPriority(
                ...
                ThreadPriority_2_kernel32_ThreadPriority[prio],
            )
    except:
