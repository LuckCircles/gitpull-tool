...
class FRectf:
    ...
    def inflate_to_square(self) -> Self:
        ...
        if w > h:
            d = w / h / 2
            p0, p1, p2 = self._mat.map(
                [FVec2f(0, 0.5 - d), FVec2f(1, 0.5 - d), FVec2f(0, 0.5 + d)]
            )
            ...
            d = h / w / 2
            p0, p1, p2 = self._mat.map(
                [FVec2f(0.5 - d, 0), FVec2f(0.5 + d, 0), FVec2f(0.5 - d, 1)]
            )
    ...
    def __str__(self):
        return f"FRectf: W:{self.width} H:{self.height} {self.as_4pts()}"
