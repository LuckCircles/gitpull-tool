...
class FEllipsef:
    ...
    def from_state(state: FDict | None) -> FEllipsef | None:
        state = FDict(state)
        if (
            (pos := FVec2f.from_state(state.get("pos", None))) is not None
            and (x_radius := state.get("x_radius", None)) is not None
            and (y_radius := state.get("y_radius", None)) is not None
            and (rotation := state.get("rotation", None)) is not None
        ):
    ...
    def __hash__(self):
        return hash((self._pos, self._width, self._height, self._rotation))
    ...
    def __eq__(self, other) -> bool:
        ...
        if isinstance(other, FEllipsef):
            return (
                ...
                & (self._x_radius == other._x_radius)
                & (self._y_radius == other._y_radius)
    ...
    def __str__(self):
        return f"FEllipsef: {self._pos} {self._rotation} {self._x_radius} {self._y_radius} {self._rotation}"
