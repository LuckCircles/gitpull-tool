...
class FWorldProj2D:
    ...
    def from_state(state: FDict | None) -> FVec2f | None:
        state = FDict(state)
        if (
            (vp_size := FVec2f.from_state(state.get("vp_size", None))) is not None
            and (w_view_pos := FVec2f.from_state(state.get("w_view_pos", None)))
            is not None
            and (scale := state.get("scale", None)) is not None
        ):
            proj = FWorldProj2D()
    ...
    def get_state(self) -> FDict:
        return FDict(
            {
                "vp_size": self._vp_size.get_state(),
    ...
    def vp2w_mat(self) -> FAffMat2:
        ...
        return (
            FAffMat2()
            .translate(-vp_size.x / 2, -vp_size.y / 2)
            .scale(scale)
            .translate(w_view_pos.x, w_view_pos.y)
        )
    ...
    def __str__(self):
        return f"FWorldProj2D {self._vp_size} {self._w_view_pos} {self._scale}"
