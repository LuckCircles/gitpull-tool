...
class FVec(FNDArray, FNDArrayArif, Sequence):
    ...
    def length(self) -> float:
        return float(math.sqrt((self._values**2).sum()))

    def dot(self, other: FVec) -> float:
        return float((self._values * other._values).sum())
    ...
    def __getitem__(self, key: int) -> int | float:
        return self._python_type(self._values[key])
