...
class FVec2(FVec):
    ...
    @property
    def yx(self) -> Tuple[int | float, int | float]:
    ...
    def atan2(self) -> float:
        return math.atan2(self._values[1], self._values[0])

    def angle(self, other: FVec2) -> float:
        return math.atan2(
            self.x * other.y - self.y * other.x, self.x * other.x + self.y * other.y
        )
    ...
    def rotate_around(self, pivot: FVec2, angle: float) -> Self:
        ...
        self._values = np.array(
            [x * c - y * s + pivot.x, x * s + y * c + pivot.y], self._dtype
        )
        return self
...
class FVec2f(FVec2):
    ...
    def from_state(state: FDict | None) -> FVec2f | None:
        return FVec2._from_state(state, FVec2f)
...
class FVec2i(FVec2):
    ...
    @staticmethod
    def from_state(state: FDict | None) -> FVec2i | None:
        return FVec2._from_state(state, FVec2i)
