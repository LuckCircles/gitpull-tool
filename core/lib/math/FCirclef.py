...
class FCirclef:
    ...
    def get_state(self) -> FDict:
        return FDict({"pos": self._pos.get_state(), "radius": self._radius})

    def is_point_inside(self, pt: FVec2f) -> bool:
        return (self._pos - pt).length <= self._radius

    def is_point_on_edge(
        self, pt: FVec2f, inner_edge_width: float, outter_edge_width: float
    ) -> float:
    ...
    def __hash__(self):
        return hash((self._pos, self._radius))
    ...
    def __str__(self):
        return f"FCirclef: {self._pos} {self._radius}"
