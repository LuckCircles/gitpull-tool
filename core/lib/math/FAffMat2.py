...
class FAffMat2(Sequence):
    ...
    @staticmethod
    def estimate(*args, **kwargs) -> FAffMat2:
        raise
    ...
    def rotate_deg(self, deg: int | float) -> Self:
        return self.rotate(deg * math.pi / 180.0)

    def rotate(self, rad: int | float) -> Self:
        alpha = math.cos(rad)
        beta = math.sin(rad)
        return self * np.float32((alpha, -beta, 0, beta, alpha, 0))
    ...
    def inverted(self) -> Self:
        ...
        result._values = np.float32(
            ((e * D, -b * D, (b * f - e * c) * D), (-d * D, a * D, (d * c - a * f) * D))
        )
    ...
    def map(self, *args):
        ...
        if isinstance(arg0, np.ndarray):
            return (
                self._values
                @ np.pad(
                    arg0, ((0, 0), (0, 1)), constant_values=(1,), mode="constant"
                ).T
            ...
            try:
                ...
                if isinstance(el0, FLine2f):
                    return tuple(
                        FLine2f(x)
                        for x in self.map(np.float32(arg0).reshape(-1, 2)).reshape(
                            -1, 2, 2
                        )
                    )
    ...
    def __mul__(self, other) -> FAffMat2:
        ...
        result._values = np.float32(
            (
                (Aa * Ba + Ad * Bb, Ab * Ba + Ae * Bb, Ac * Ba + Af * Bb + Bc),
                (Aa * Bd + Ad * Be, Ab * Bd + Ae * Be, Ac * Bd + Af * Be + Bf),
            )
        )
