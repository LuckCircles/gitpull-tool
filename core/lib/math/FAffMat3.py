...
class FAffMat3:
    ...
    @staticmethod
    def estimate(*args, **kwargs) -> FAffMat3:
        raise
    ...
    def __mul__(self, other) -> FAffMat3:
        return FAffMat3(
            (
                np.concatenate([self._values, np.float32([[0, 0, 0, 1]])], 0)
                @ np.concatenate([other._values, np.float32([[0, 0, 0, 1]])], 0)
            )[:3]
        )
