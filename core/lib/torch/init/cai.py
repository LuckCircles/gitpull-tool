...
def init_cai_conv2d(weight, eps_std=0.05):
    ...
    if basis_size == 1:
        x = torch.normal(
            0.0, eps_std, (out_ch, in_ch, basis_size), dtype=dtype, device=device
        )
    else:
        nbb = in_ch // basis_size + 1
        x = torch.normal(
            0.0, 1.0, (out_ch, nbb, basis_size, basis_size), dtype=dtype, device=device
        )
        x = x * (1 - torch.eye(basis_size, dtype=dtype, device=device)) + torch.permute(
            x, (0, 1, 3, 2)
        )
