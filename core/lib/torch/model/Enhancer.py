...
class Enhancer(nn.Module):
    def __init__(
        ...
        for up_ch, down_ch in list(zip(dims[:-1], dims[1:])):
            ...
            up_s_list.insert(
                0,
                nn.Sequential(
                    nn.Conv2d(down_ch, up_ch * 4, 3, 1, 1),
                    nn.LeakyReLU(0.2),
                    nn.PixelShuffle(2),
    ...
    def forward(self, inp):
        ...
        shortcuts = []
        for down_c1, down_c2, down_p in zip(
            self._down_c1_list, self._down_c2_list, self._down_p_list
        ):
            x = down_c1(x)
            x = down_c2(x)
            x = down_p(x)
        ...
        for shortcut_x, up_c1, up_s, up_r, up_c2 in zip(
            shortcuts,
            self._up_c1_list,
            self._up_s_list,
            self._up_r_list,
