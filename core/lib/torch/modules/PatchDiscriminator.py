...
class PatchDiscriminator(nn.Module):
    def __init__(
        self, in_ch: int, out_ch: int, patch_size: int, base_dim=32, max_downs=5
    ):
        super().__init__()

        layers_cfg = RFA.create(
            layers=max_downs, k_list=(3, 5, 7), s_list=(1, 2)
        ).nearest(patch_size)

        dim_mults = [min(2**i, 8) for i in range(layers_cfg.layers_count + 1)]
        dims = [base_dim * mult for mult in dim_mults]

        self._in_beta = nn.parameter.Parameter(
            torch.zeros(
                in_ch,
            ),
            requires_grad=True,
        )
        self._in_gamma = nn.parameter.Parameter(
            torch.ones(
                in_ch,
            ),
            requires_grad=True,
        ...
        for level, (rfa_layer, up_ch, down_ch) in enumerate(
            list(zip(layers_cfg, dims[:-1], dims[1:]))
        ):
    ...
    def forward(self, inp: torch.Tensor):
        ...
        for shortcut_x, up_c, up_s, logit in zip(
            shortcuts, self._up_c_list, self._up_s_list, self._logits_list
        ):
