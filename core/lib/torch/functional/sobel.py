...
def rot_matrix(m, diff):
    ...
    for ring in range(0, W // 2):
        ...
        T = m[ring, ring : W - ring]
        T_len = len(T)
        R = m[ring + 1 : W - ring :, W - ring - 1]
        R_len = len(R)
        B = m[W - ring - 1, ring : W - ring - 1][::-1]
        B_len = len(B)
        L = m[ring + 1 : W - ring - 1 :, ring][::-1]
        L_len = len(L)
        ...
        out[W - ring - 1, ring : W - ring - 1][::-1] = x[
            T_len + R_len : T_len + R_len + B_len
        ]
...
def get_sobel_kernel(ch, kernel_size=5, diagonals=True, dtype=np.float32) -> np.ndarray:
    ...
    sobel_2D_denominator[kernel_size // 2, kernel_size // 2] = (
    ...
    k = np.concatenate(
        [kx[None, ...], ky[None, ...]]
        + (
            [rot_matrix(kx, 1)[None, ...], rot_matrix(ky, 1)[None, ...]]
            if diagonals
            else []
        ),
        0,
...
def sobel_edges_2d(
    x: torch.Tensor, kernel_size=5, flat_ch=False, blur=False, norm=False
):
    """
    ...
    x_sobel = F.conv2d(
        F.pad(x, (kernel_size // 2,) * 4, mode="reflect"),
        kernel_t,
        stride=1,
        padding=0,
        groups=C,
