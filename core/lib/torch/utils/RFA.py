...
class RFA:
    ...
    class LayersConfig(List[Layer], list):
        ...
        @cached_property
        def kernel_size_sum(self) -> int:
            return sum(l.kernel_size for l in self)

        @cached_property
        def stride_sum(self) -> int:
            return sum(l.stride for l in self)

        def __repr__(self):
            return self.__str__()

        def __str__(self):
            return f"LayersConfig, RFS:{self.rfs}, " + " ".join(
                f"({l.kernel_size}, {l.stride})" for l in self
            )

    @staticmethod
    def create(
        layers: int | Sequence[int] = 5,
        k_list: Sequence[int] = (3, 5, 7),
        s_list: Sequence[int] = (1, 2),
        prefer_more_layers: bool = True,
    ) -> RFA:
            ...
            if len(cfg_list) == 1:
                ...
                for cfg in cfg_list:
                    if (
                        (
                            prefer_more_layers
                            and cfg.layers_count > best_cfg.layers_count
                        )
                        or (
                            not prefer_more_layers
                            and cfg.layers_count <= best_cfg.layers_count
                        )
                        or (
                            cfg.kernel_size_sum <= best_cfg.kernel_size_sum
                            and cfg.stride_sum >= best_cfg.stride_sum
                        )
                    ):
                        best_cfg = cfg
    ...
    def min_rfs(
        ...
        return sorted(self._d.keys())[0]

    @property
    def max_rfs(
        self,
    ) -> int:
        return sorted(self._d.keys())[-1]
    ...
    def _gen_layers_config(
        layer_count: int,
        k_list,
        s_list,
        layer_id: int = 0,
    ) -> Generator[RFA.LayersConfig]:
        if layer_id == layer_count:
            yield RFA.LayersConfig()
        else:
            for ks in k_list:
                for s in s_list:
                    for sub in RFA._gen_layers_config(
                        layer_count, k_list, s_list, layer_id + 1
                    ):
                        yield RFA.LayersConfig(
                            (RFA.Layer(kernel_size=ks, stride=s),) + tuple(sub)
                        )
