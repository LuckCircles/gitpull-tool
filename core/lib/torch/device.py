...
class Device:
    ...
    def get_state(self) -> FDict:
        return FDict({"type": self._type, "index": self._index})

    def __hash__(self):
        return (self._type, self._index).__hash__()

    def __eq__(self, other):
        if isinstance(self, (Device, torch.device)) and isinstance(
            other, (Device, torch.device)
        ):
...
def get_cpu_device_info() -> DeviceInfo:
    return DeviceInfo(type="cpu", index=None, name="CPU", total_memory=0)
...
def get_cpu_device() -> Device:
    return Device(type="cpu", index=None)


def get_avail_gpu_devices() -> List[Device]:
    return [
        Device(type=dev.type, index=dev.index) for dev in get_avail_gpu_devices_info()
    ]


def get_avail_devices() -> List[Device]:
    return [get_cpu_device()] + get_avail_gpu_devices()
