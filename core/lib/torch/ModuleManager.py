...
class ModuleManager:
    ...
    def set_optimizer_factory(
        self, func: Callable[[ModuleManager, nn.Module], opt.Optimizer]
    ):
    ...
    def _get_record(self, key: Any) -> ModuleManager._Record:
        ...
        if record is None:
            ...
            if (
                model_state_bytes := self._initial_state.get(state_key, None)
            ) is not None:
                model_state = torch.load(
                    io.BytesIO(model_state_bytes), map_location="cpu", weights_only=True
                )
            else:
                model_state = None
            ...
            # Create record
            record = self._records[key] = ModuleManager._Record(
                module=module, opt=None, device=get_cpu_device(), train=False
            )
        return record

    def get_module(
        self, key: Any, device: Device = None, train: bool = None
    ) -> nn.Module:
        """
    ...
    def get_optimizer(self, key: Any) -> nn.Module:
        ...
        if record.opt is None:
            ...
            if (
                opt_state_bytes := self._initial_state.get(state_key, None)
            ) is not None:
                opt_state = torch.load(
                    io.BytesIO(opt_state_bytes), map_location="cpu", weights_only=True
                )
            else:
                opt_state = None
