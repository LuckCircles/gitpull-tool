...
class Task[T](Future[T]):
    ...
    def alive_time(self) -> float:
        return datetime.now().timestamp() - self._creation_time

    def attach_to(
        self, fg: FutureGroup, max_tasks: int = None, detach_parent: bool = True
    ) -> bool:
        """
    ...
    def _on_finish(self):
        ...
        if self._gen is not None:
            try:
                self._gen.throw(
                    TaskFinishException(
                        self, error=self.error if not self.succeeded else None
                    )
                )
                ...
                if self._gen_next:
                    ...
                    try:
                        ...
                        if g_log.get_level() >= 1:
                            print(
                                f"Unhandled exception {e} occured during execution of task {self}. Traceback:\n{traceback.format_exc()}"
                            )
                        exec_stack.pop()
                        self.cancel(error=e)
                        break
                    ...
                    if gen_yield_t is wait:
                        ...
                        if self.attach_to(
                            gen_yield._fg,
                            max_tasks=gen_yield._max_tasks,
                            detach_parent=gen_yield._detach_parent,
                        ):
                            self._gen_next = True
                        else:
                            self.cancel()
                        ...
                        gen_yield._future.call_on_finish(
                            lambda other_future: (
                                self.success(other_future.result)
                                if other_future.succeeded
                                else self.cancel(error=other_future.error)
                            )
                        )
                        break
...
class TaskFinishException(Exception):
    ...
    def __str__(self):
        return f"TaskFinishException of {self._task}"
