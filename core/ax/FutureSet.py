...
class FutureSet(Generic[T]):
    ...
    def count_finished(self) -> int:
        return sum(1 for fut in self._futs if fut.finished)

    @property
    def count_succeeded(self) -> int:
        return sum(1 for fut in self._futs if fut.finished and fut.succeeded)

    @property
    def count_cancelled(self) -> int:
        return sum(1 for fut in self._futs if fut.finished and not fut.succeeded)

    @property
    def empty(self) -> bool:
        return len(self._futs) == 0

    def add(self, fut: Future[T]):
        self._futs.add(fut)

    def remove(self, fut: Future[T]):
