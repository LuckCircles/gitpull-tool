...
class Thread(mx.Disposable):
    ...
    def __init__(self, name: str = None, priority: Priority = None, **kwargs):
        ...
        self._name = (
            name if name is not None else f"Unnamed #{next(Thread._unnamed_counter)}"
        )
        ...
        if not self._disposing_ev.is_set():
            ...
            if self._spawned:
                if threading.get_ident() == self._ident:
                    raise Exception(
                        "Unable to dispose spawned Thread while executing in this Thread. Switch to other Thread and then call dispose."
                    )
                else:
                ...
                if threading.get_ident() != self._ident:
                    raise Exception(
                        "non-spawned Thread.dispose() must be called from the same OS thread."
                    )
    ...
    def get_tls(self) -> ThreadLocalStorage:
        return ThreadLocalStorage._by_ident[self._ident]
    ...
    def execute_tasks_once(self):
        ...
        if threading.get_ident() != self._ident:
            raise Exception(
                "execute_tasks_once must be called from OS thread where the Thread was spawned/registered."
            )
    ...
    def execute_tasks_loop(
        self, exit_if: Callable[[], bool] = None, max_time: float = None
    ):
        """
    ...
    def get_printable_info(self, include_tasks=False) -> str:
        ...
        if include_tasks:
            with self._lock:
                if self._active_tasks is not None:
                    active_tasks = tuple(
                        task for task in self._active_tasks if not task.finished
                    )
...
class _Callsoon(mx.Disposable):
    ...
    def __init__(self, func: Callable):
        ...
        mx.CallOnDispose(lambda: setattr(self, "_callable", None)).dispose_with(
            self._callable
        )
